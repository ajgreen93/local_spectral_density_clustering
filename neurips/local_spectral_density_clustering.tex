...
\end{equation*}
where $\widetilde{\pi}_{\min} = \min_{u \in \Csig[\Xbf]} \widetilde{\pi}_{n,r}(u)$. As $\widetilde{\pi}_{\min} = \mathcal{O}(1/n)$, the upper bound on the right hand side will be of order at least $\log(n)$, and the bound becomes trivial as $n \to \infty$. 
