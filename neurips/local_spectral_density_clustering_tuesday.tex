...
\end{equation}
where $\cut(S;G_{n,r}) = \sum_{u \in S, v \in S^c} \Abf_{uv}$ and $\vol(S; G_{n,r}) := \sum_{u \in S} \Dbf_{uu}$. Having computed sweep cuts $S_{\beta}$ over a range \smash{$\beta \in  
...
originating at $v$. Also write \smash{$\pi = (\pi_u)_{u \in S}$}
