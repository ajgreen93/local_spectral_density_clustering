...
Denote $\widetilde{G}_{n,r} = G_{n,r}[\Csig[\Xbf]]$; in general, we will use tilde notation to refer to quantities computed over $\Csig$ or over the induced subgraph $\widetilde{G}_{n,r}$. The goal of Theorem \ref{thm: mixing_time_upper_bound} is to show that, for universal constants $c_2, c_3,c_4 > 0$, with probability one
\begin{equation*}
...
In the first step, we relate the mixing time $\tau_{\infty}(G)$ of a generic undirected graph $G = (V,E)$ to expansion properties of subsets $U \subseteq V$. We now build to a formal definition of these expansion properties. First, we recall the the \emph{cut} and \emph{volume} functionals over a graph, and introduce the \emph{degree} functional as well. For $u \in V$, $S \subseteq V$, 
\begin{equation*}
...
The second step in our approach is to lower bound the local spread and conductance over the particular graph $\widetilde{G}_{n,r}$. The following result provides the desired lower bounds, which hold asymptotically as $n \to \infty$,
\begin{proposition}
...
Taking these results as given, the proof of the theorem is more or less complete. We note that since $r$ is fixed, \textcolor{red}{with probability one} $\liminf_{n \to \infty} \min_{u \in \Csig[\Xbf]} \deg(u; \widetilde{G}_{n,r}) = \infty > 10$, and we may therefore apply Proposition \ref{prop: pointwise_mixing_time} with respect to $\widetilde{G}_{n,r}$. Along with Lemma \ref{prop: local_spread_conductance}, this yields
\begin{equation*}
\tau_{\infty}(\widetilde{G}_{n,r}) \leq \frac{2^{27}\Lambda_{\sigma}^4\rho^2L^2 d^3}{\lambda_{\sigma}^4 r^2} \log^2\left(\frac{28800\Lambda_{\sigma}}{\lambda_{\sigma}^2 r}\right) + 3 \log^2\left(\frac{280\Lambda_{\sigma}}{\lambda_{\sigma}^2 r}\right) + 3
\end{equation*}
The first term dominates the middle term, and the theorem follows after an appropriate choice of $c_2$ and $c_3$.

\subsection{Proof of Proposition \ref{prop: pointwise_mixing_time}}
\label{sec: pointwise_mixing_time}

We first generalize some notation from the main text. Let $A$ be the adjacency matrix of an undirected graph $G = (V,E)$, and $D$ the associated diagonal degree matrix. The lazy random walk over $G$ is the Markov chain with transition probabilities given by $\Wbf := \frac{\Ibf{} + \Dbf^{-1} \Abf}{2}$ and stationary distribution $\pi$.  Denote the $t$-step probability distribution of this random walk, originating from a particular $v \in V$, as $q^{(t)}: V \times V \to [0,1]$, $q^{(t)}(v,u) = e_v \Wbf^t e_u$.

Consider the uniform distance\footnote{Note $d_{\textrm{unif}}$ is not formally a distance as it is not symmetric.} between $q_v^{(t)} := q^{(t)}(v,\cdot)$ and $\pi$, given by
\begin{equation*}
d_{\textrm{unif}}(q_v^{(t)},\pi) = \max_{u \in V} \set{\frac{\pi(u) - q_v^{(t)}(u)}{\pi(u)}}.
\end{equation*}
Our goal is to demonstrate that for sufficiently large $t$, $d_{\textrm{unif}}(q_v^{(t)},\pi) \leq \frac{1}{4}$ for every $v \in V$. To do so, we introduce the \emph{total variation distance} between the distributions $q_v^{(t)}$ and $\pi$, given by
\begin{equation*}
...
\end{equation*}
the uniform distance $d_{\textrm{unif}}(q_v^{(\tau_1 + 3)}, \pi) \leq \frac{1}{4}$, which proves the claim of Proposition \ref{prop: pointwise_mixing_time}. Fix $a = \frac{1}{18}$, and let $\tau_0 = \frac{2}{\Phi^2(G)} \log \left(\frac{80}{a s(G)}\right)$. Then,
\begin{equation*}
\left(1 - \frac{\Phi^2(G)}{2}\right)^{\tau_0} \leq \exp(- \tau_0\Phi^2(G)/2) \leq \frac{as(G)}{80}
\end{equation*}
and so by Lemma \ref{lem: tv_mixing_time},
\begin{equation*}
...
\end{equation*}
and so by Lemma \ref{lem: tv_to_uniform_distance}, $d_{\textrm{unif}}(q_v^{(\tau_1 + 3)}, \pi) \leq \frac{1}{4}$. The proof is complete once we have proved Lemmas \ref{lem: tv_mixing_time} and \ref{lem: tv_to_uniform_distance}.
...
We will find it useful to introduce the \emph{Lovasz-Simonovits curve}, defined as
$h_z^{(t)}, t \geq 0$, 
\begin{equation*}
h_q^{(t)}(x) = \max_{w \in \mathcal{W}_x} \left\{ \sum_{u \in V} \left(q_z^{(t)}(u) - \pi(u)\right)w(u)\right\}
\end{equation*}
...
\begin{equation}
\label{eqn: lovasz_simonivits_1}
\max \set{h_v^{(t)}(\pi(S)), h_v^{(t)}(1 - \pi(S))} \geq \abs{q_v^{(t)}(S) - \pi(S)}
\end{equation}
Therefore, taking the maximum on both sides of \eqref{eqn: lovasz_simonivits_1}  over all $S \subseteq V$, we have
\begin{equation*}
\norm{q_v^{(t)} - \pi}_{\mathrm{TV}} \leq \max_{0 \leq x \leq 1} h_v^{(t)}(x).
\end{equation*}
Now, take $z = e_vW^3$. Observe that $h_v^{(t+3)}(x) = h_z^{(t)}(x)$. To prove Lemma \ref{lem: tv_mixing_time}, it is therefore sufficient to show the desired upper bound holds with respect to $h_z^{(t)}(x)$ for all $x \in [0,1]$. 

To show this upper bound, we split the interval $[0,1]$ into the subinterval $[\mu, 1 - \mu]$ and the remainder $[0,\mu) \cup (1 - \mu, 1]$, where $\mu = as(G)$. Let $\ell_{\mu}(x)$ be the linear interpolator between $h_z^{(0)}(\mu)$ and $h_z^{(0)}(1 - \mu)$, 
\begin{equation*}
...
& \leq \max\set{as(G), \frac{1}{2^t} + \frac{9a}{20} + \frac{1}{2 \min_{u \in V}\deg(u;G)} }
\end{align*}
with the latter inequality following from \eqref{eqn: mixing_small_sets}. Therefore, for any $x \in [0,1]$,
\begin{equation*}
h_z^{(t)}(x) \leq \max\set{as(G), \frac{1}{2^t} + \frac{9a}{20} + \frac{1}{2 \min_{u \in V}\deg(u;G)} } + \left(\frac{1}{1 - 2as(G)} + \frac{1}{as(G)}\right)\left(1 - \frac{\Phi^2(G)}{2}\right)^t
\end{equation*}
...
\begin{theorem}[Theorem 1.2 of \cite{lovasz1990}]
	...
	\end{equation}
\end{theorem}
In order to show \eqref{eqn: mixing_large_sets}, we must therefore provide an appropriate bound on the quantity $C_{\mu}$. Precisely, we will show
\begin{equation}
\label{eqn: lt_ub_1}
...
It remains to show \eqref{eqn: lt_ub_1}. To do so, we make use of an equivalent representation of the Lovasz-Simonovits curve $h_z^{(t)}$. Order the elements of $V = \set{u_1, \ldots, u_N}$ ($N$ = $\abs{V}$), such that
\begin{equation*}
\frac{q_z^{(t)}(u_1)}{\pi(u_1)} \geq \frac{q_z^{(t)}(u_2)}{\pi(u_2)} \geq \ldots \geq \frac{q_z^{(t)}(u_N)}{\pi(u_N)}
\end{equation*}
and let $U_k = \set{u_1, \ldots, u_k}$. Then for any $x$, letting $k$ satisfy $\pi(U_{k - 1}) < x < \pi(U_k)$, it can be shown that\footnote{See \cite{lovasz1990} for a formal justification.},
\begin{equation}
...
\end{equation}
The representation of the Lovasz-Simonovits curve given by \eqref{eqn: lovasz_simonovits_curve} makes it clear that $h_z^{(t)}(x)$ is a concave, piecewise linear curve. Moreover, we observe that the slope $v(x)$ of each linear segment of the Lovasz-Simonovits curve satisfies
\begin{equation}
\label{eqn: lovasz_simonovits_slope}
-1 \leq v(x) \leq \frac{h_z^{(0)}(\mu)}{\mu}.
\end{equation}
Note that $\ell_{\mu}(\mu) = h_z^{(0)}(\mu)$, and for $x \geq \mu$, 
\begin{equation*}
...
\end{equation}
Recalling that $z = e_vW^3$, we will rely on the key fact that for any $u \neq v, t \geq 1$,
\begin{equation}
...
q_z^{(m)}(u) \leq \frac{1}{2^{t + 3}} + \frac{1}{2\min_{u \in V}\deg(u;G)}.
\end{equation}
...
\begin{equation*}
h_z^{(t)}(x) \leq \frac{1}{2^{t + 3}} + \frac{\abs{U_k}}{2 \min_{u \in V}\deg(u;G)} + \frac{1}{2 \min_{u \in V}\deg(u;G)}.
\end{equation*}
Finally, since $x \leq a s(G)$, 
\begin{equation*}
\abs{U_k} \leq \frac{x}{10 \min_{u \in V}(\pi(v))} \leq \frac{9a\min_{u \in V}\deg(u;G)}{10}.
\end{equation*}
and the desired result is shown.

Now, we turn to the case where $x \geq 1 - as(G)$. But since $h_z^{(t)}(1) = 0$, and the slope of $h_z$, $v(x')$ is no less than $-1$ for all $x' \in [0,1]$, this implies $h_z^{(t)}(x) \leq 1 - x \leq as(G)$.

\subsection{Proof of Lemma \ref{lem: tv_to_uniform_distance}}

The proof of Lemma \ref{lem: tv_to_uniform_distance} follows from straightforward algebra. Fix $u \in V$ and let $m \geq t + 1$ be arbitrary. The stationarity of $\pi$ gives
\begin{align}
...
We first prove \eqref{eqn: local_spread} and then \eqref{eqn: conductance}. First, we introduce some notation. Recall that $\widetilde{G}_{n,r} = G_{n,r}\bigl[\Csig[\Xbf]\bigr]$ denotes the subgraph induced by $\Csig[\Xbf]$. For $S \subseteq \Csig[\Xbf]$ and $u \in \Csig[\Xbf]$, we will write
\begin{equation*}
...
\begin{equation*}
\liminf_{n \to \infty} \min_{S \subseteq \Csig[\Xbf]} \widetilde{\Phi}_{n,r}(S) \geq \frac{\lambda_{\sigma}^2 r}{\Lambda_{\sigma}^2 2^{13} \rho L \sqrt{d}}.
\end{equation*}
where we let $S^c = \Csig[\Xbf] \setminus S$. In order to show this bound holds uniformly over all sets $S \subseteq \Csig[\Xbf]$, we will split the analysis into two cases based on the size of $S \subseteq \Csig[\Xbf]$. To do so, we introduce $\mathcal{L}(G) := \set{S \subseteq V: \pi(S) \geq s(G)}$ (where as usual $\pi$ denotes the stationary distribution of a random walk over $G = (V,E)$.)

Lemma \ref{lem: graph_conductance_profile_lb_1} shows that for sets $S \subseteq \Csig[\Xbf]$ not sufficiently large enough to be in $\mathcal{L}(\widetilde{G}_{n,r})$, the graph normalized cut is of constant order. In fact, this statement holds for any graph $G$.
\begin{lemma}
...
Taking Lemma \ref{lem: graph_conductance_profile_lb} as given, Proposition \ref{prop: graph_conductance_profile_lb} then follows from that Lemma, along with Lemma \ref{lem: graph_conductance_profile_lb_1}, in light of the fact $\frac{\lambda_{\sigma}^2 r}{\Lambda_{\sigma}^2 2^{13} \rho L \sqrt{d}} < \frac{1}{10}$. We turn now to the proof of Lemma \ref{lem: graph_conductance_profile_lb}.

\subsection{Proof of Lemma \ref{lem: graph_conductance_profile_lb}.}

Let $\Sset \subseteq \Csig$ and $S \subseteq \Csig[\Xbf]$. The proof of \ref{lem: graph_conductance_profile_lb} will essentially follow from a pair of technical results. The first of these will demonstrate that the functional $\widetilde{\Phi}_{n,r}(S)$ can be lower bounded by a population analogue $\widetilde{\Phi}_{\Pbb,r}(\Sset)$, which we term the continuous normalized cut. This lower bound will hold uniformly over sets $S \subseteq \Csig[X]$. The second technical Lemma will build on known continuous space isoperimetric inequalities to lower bound the population-level quantity $\widetilde{\Phi}_{\Pbb,r}(\Sset)$ uniformly over sets $\Sset \subseteq \Csig$.

We will now build slowly toward a formal definition of the continuous normalized cut, before establishing a relation between it and it's discrete counterpart. Let $\Sset, \Sset_1, \Sset_2 \subseteq \Csig$ be measurable. We introduce the \emph{$r$-ball walk}, a Markov chain over $\Csig$ with transition probability at $x \in \Csig$ given by 
\begin{equation*}
...
As the functionals $\widetilde{\Phi}_{n,r}$ and $\widetilde{\Phi}_{\Pbb,r}$ act in the different spaces $\Csig[\Xbf]$ and $\Csig$, respectively, it is not obvious how to relate them. To do so, we introduce transportation maps between the space $\Csig$ and the sample points $\Csig[\Xbf]$. Precisely, define the conditional probability measures
\begin{equation*}
\widetilde{\Pbb}(\Sset) = \frac{\Pbb(\Sset)}{\Pbb(\Csig)}, ~ \widetilde{\Pbb}_{n}(\Sset) := \frac{1}{\abs{\Csig[\Xbf]}} \sum_{x_i \in \Csig[\Xbf]} \1(x_i \in \Sset).
\end{equation*} 
A measurable map $T: \Csig \to \Csig[\Xbf]$ is said to be a \emph{transportation map} between $\widetilde{\Pbb}$ and $\widetilde{\Pbb}_n$ if for any $S \subseteq \Csig[\Xbf]$
\begin{equation*}
\widetilde{\Pbb}(T^{-1}(S)) = \widetilde{\Pbb}_n(S).
\end{equation*}
where $T^{-1}(S) = \set{x \in \Csig: T(x) \in S}$ is the preimage of $T$. The following result lower bounds $\widetilde{\Phi}_{n,r}(S)$ by $\widetilde{\Phi}_{\Pbb,r}(T^{-1}(S))$, plus some perturbation. The magnitude of this perturbation, and thus the quality of the bound, will be dictated by the \emph{transportation distance} $\norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})}$, where $\mathrm{Id}(x) = x$ is the identity mapping over $\Csig$. 
\begin{lemma}
	\label{lem: graph_to_continuous_conductance}
	Let $\seq{T_n}$ be a sequence of transportation maps between $\widetilde{\Pbb}$ and $\widetilde{\Pbb}_n$.Then, letting $\Sset =T_n^{-1}(S)$, there exists a constant $c > 0$ fixed in sample size $n$ such that for any $S \in \mathcal{L}(\widetilde{G}_{n,r})$
	\begin{equation}
	\label{eqn: graph_to_continuous_conductance}
	\widetilde{\Phi}_{n,r}(S) \geq \widetilde{\Phi}_{\Pbb,r}(\Sset)\left(1 - c \norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})}\right)
	\end{equation}
\end{lemma}

(We note that hereafter in this section $c$ will represent a constant which may change from line to line, and may depend on the density $f$ and dimension $d$, but not on the sample size $n$). As mentioned previously, Lemma \ref{lem: graph_to_continuous_conductance} is useful only in conjunction with an upper bound on the transportation distance $\norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})}$. Proposition 5 of \cite{garciatrillos16} establishes such an upper bound, with respect to transportation maps on measures compactly supported over open connected domains with Lipschitz boundaries. The following result is a restatement of this Proposition with respect to the domain $\Csig$, and the measure $\widetilde{\Pbb}$. Although $\Csig$ is closed rather than open, as $\nu(\partial \Csig) = 0$ we may apply Proposition 5 of \cite{garciatrillos16} to the interior $\Csig^o$ of $\Csig$, and the desired result will hold for any arbitrary extension of $T_n$ to $\Csig$. Let $\widetilde{n} = \abs{\Csig{\Xbf}}$. 
...
The following result then supplies the last step, a uniform lower bound on the continuous normalized cut $\widetilde{\Phi}_{\Pbb,r}(\Sset)$ for all sets $\Sset \subseteq \Csig$. Let the \emph{$\Pbb$-(continuous) conductance} be given by
\begin{equation*}
\widetilde{\Phi}_{\Pbb,r} := \min_{\Sset \subseteq \Csig} \widetilde{\Phi}_{\Pbb,r}(\Sset)
\end{equation*}
where $\Sset^c = \Csig \setminus \Sset$, and the minimum in the preceding display is taken over measurable sets $\Sset \subseteq \Csig$. 
\begin{lemma}
	\label{lem: nonuniform_continuous_conductance}
	For any $0 < r < \frac{\sigma}{2\sqrt{d}}$, the $\Pbb$-continuous conductance of the $r$-ball walk satisfies
	\begin{equation*}
	\widetilde{\Phi}_{\Pbb,r} > \frac{\lambda_{\sigma}^2 r}{2^{13} \Lambda_{\sigma}^2 \rho L \sqrt{d}}.
	\end{equation*}
\end{lemma}

With Lemmas \ref{lem: graph_to_continuous_conductance} and \ref{lem: nonuniform_continuous_conductance}, as well as Theorem \ref{thm: stagnating_transportation_maps}, in hand, we proceed to the proof of Lemma \ref{lem: graph_conductance_profile_lb}, which follows straightforwardly:
\begin{align*}
\liminf_{n \to \infty} \set{\min_{S \in \mathcal{L}(\widetilde{G}_{n,r})} \widetilde{\Phi}_{n,r}(S)} & \geq \liminf_{n \to \infty} \set{ \min_{\Sset \in \Csig} \widetilde{\Phi}_{\Pbb,r}(\Sset)\left(1 - c \norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})}\right) } \\
& \geq \liminf_{n \to \infty} \set{ \min_{\Sset \in \Csig} \widetilde{\Phi}_{\Pbb,r}} - \limsup_{n \to \infty} c \norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})} \\
& \geq \frac{\lambda_{\sigma}^2 r}{2^{13} \Lambda_{\sigma}^2 \rho L \sqrt{d}}.
\end{align*}

It remains to prove Lemmas \ref{lem: graph_to_continuous_conductance} and \ref{lem: nonuniform_continuous_conductance}, which we turn to in the following two subsections.


\subsection{Population-level conductance.}
\label{sec: population_conductance_function}

We turn now to lower bounding the conductance over $\widetilde{G}_{n,r} := G_{n,r}[\Csig[\Xbf]]$ (in general, we will use tilde notation to refer to quantities computed over $\Csig$ or over the induced subgraph $\widetilde{G}_{n,r}$). Our first step will be to introduce a population analogue  of the conductance, and prove a lower bound on this quantity. 
...
For $\Sset \subseteq \Csig$ measurable, 

If a sequence of transportation maps $\seq{T_n}$ satisfies $\lim_{n \to \infty}\norm{\mathrm{Id} - T_n}_{L^{\infty}(\widetilde{\Pbb})} = 0$ (where $\mathrm{Id}$ is the identity mapping), we refer to it as a sequence of \emph{stagnating transportation maps}. 
