...
Denote $\widetilde{G}_{n,r} = G_{n,r}[\Csig[\Xbf]]$; in general, we will use tilde notation to refer to quantities computed over $\Csig$ or over the induced subgraph $\widetilde{G}_{n,r}$. The goal of Theorem \ref{thm: mixing_time_upper_bound} is to show that, for universal constants $c_2, c_3 > 0$, with probability one as $n \to \infty$,
\begin{equation*}
\limsup_{n \to \infty} \tau_{\infty}(\widetilde{G}_{n,r}) \leq c_2 \frac{\Lambda_{\sigma}^4 d^3 \rho^2 L^2}{\lambda_{\sigma}^4 r^2} \log^2\left(\frac{1}{r}\right) + c_3
\end{equation*}
We follow a two-step approach typically to used to establish upper bounds on the mixing time of Markov chains.

In the first step, we relate the mixing time $\tau_{\infty}(G)$ of a generic graph $G = (V,E)$ to expansion properties of subsets $U \subseteq V$. We now build to a formal definition of these expansion properties. First, we recall the the \emph{cut} and \emph{volume} functionals over a graph, and introduce the \emph{degree} functional as well. For $u \in V$, $S \subseteq V$, 
\begin{equation*}
\cut(S;G) = \sum_{u \in S} \sum_{v \in S^c} \1((u,v) \in E), \quad \deg(u;G) = \sum_{v \in V} \1((u,v) \in E), \quad \vol(S;G) = \sum_{u \in S} \deg(u;G).
\end{equation*}
Additionally, we recall the \emph{normalized cut} $\Phi(S;G)$ is defined (as in \eqref{eqn: normalized_cut}) as
\begin{equation*}
...
Taking these Lemmas as given, we proceed to prove the proposition. Fix $a = \frac{1}{18}$, and let $\tau_0 = \frac{2}{\Phi^2(G)} \log \left(\frac{80}{a s(G)}\right)$. Then,
\begin{equation*}
...
We generalize the notation of the previous subsection. For a starting distribution $z$ to be specified later (meaning $\textrm{supp}(z) \subseteq V$ and $\sum_{u \in V}z(u) = 1$), and for $t \geq 0$ an integer, let $q_z^{(t)}$ be the $t$-step probability distribution of the lazy random walk with starting distribution $z$.
...
\end{align*}
The utility of the Lovasz-Simonovits curve comes from the following observations:
first, note that
\begin{equation*}
...
\end{equation*}
Therefore, taking the maximum over all $S \subseteq V$ on both sides, we have
\begin{equation*}
...
To do so, we split the interval $[0,1]$--based on a number $\mu = as(G)$ for some $0 < a < 1$ to be specified later--into the subinterval $[\mu, 1 - \mu]$ and the remainder $[0,\mu) \cup (1 - \mu, 1]$. Let $\ell_{\mu}(x)$ be the linear interpolator between $h_z^{(0)}(\mu)$ and $h_z^{(0)}(1 - \mu)$, 
\begin{equation*}
...
\begin{lemma}
	...
	\end{equation}
\end{lemma}
\noindent Taking Lemma \ref{lem: mixing_all_sets} as given, we have nearly completed our proof. Note that for any $\mu \leq x \leq 1 - \mu$,
\begin{align*}
