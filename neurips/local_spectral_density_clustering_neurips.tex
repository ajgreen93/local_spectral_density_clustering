...
with the upper bound in \ref{asmp: embedding}, this leads to an upper bound on
...
analogous mixing time bounds developed for the mixing time of the
...
algorithms. The first column shows the empirical density clusters $C[X]$
and $C'[X]'$ for a particular threshold $\lambda$ of the density function; the 
...
interesting to establish that it \emph{only} recovers such clusters, a point we
return to below.   
...
cluster by PPR. We do not, however, provide a theoretical lower bound showing
that our geometric conditions are required for successful recovery on an upper
level set. Although we investigate the matter empirically, this is a direction
for future work.
