...
Our analysis naturally builds on a few of the aforementioned theoretical analyses of PPR. For an arbitrary graph $G$ and subset $S \subset G$, \citet{andersen2006} relate the quality of the PPR cluster $\hat{C}$ to the normalized cut functional $\Phi(S;G)$. While this analysis is tight, it fails to account for improvements possible when the cluster $S$ is additionally assumed to be internally well-connected, which is intuitively a more favorable case for clustering. Building on this intuition, \citet{zhu2013} assume that the subgraph $G[S]$ is internally well-connected-- measured by a functional such as mixing time of a random walk over $G[S]$-- and prove upper bounds on $\vol(\widehat{C} \vartriangle S;G)$. Both of these analyzes hold with respect to a approximate form of PPR (APPR) which can be efficiently computed.


We apply these results to our setting by estimating these functionals in the particular case of $G = G_{n,r}$ and $S = \mathcal{C}[X]$. In particular, we prove an upper bound on the mixing time of a random walk run only over the subset of nodes in $G_{n,r}$ which fall within a density cluster $\Cset$. To do so, we rely on a series of seminal works upper bounding the mixing time of \emph{geometric random walks} (see \citet{vempala2005} for a comprehensive review.)  This study was initiated by \citet{dyer1991}, who used geometric random walks as a fundamental subroutine to efficiently compute volumes of high-dimensional convex bodies. These results are improved in \citet{lovasz1990,kannan97,kannan06}, who show, inter alia, that the bounds on mixing time can be sharpened by avoiding so-called ``start-penalties''. As we discuss further in the sequel, these improvements are crucial to our work. Following the work of \citet{abbasi-yadkori2016,abbasi-yadkori2016a}, we extend these results to hold for Lipschitz deformations of convex sets. Additionally, we relate the mixing time of these (continuous-space) geometric random walks to the mixing time of random walks over (discrete) neighborhood graphs. 
...
$c(\Pbb,r), c_i(\Pbb,r) > 0$ represent constants which may depend on $\Pbb,\lambda$ and $r$ but not on the sample size $n$; where possible, we keep track of all constants in our proofs.
\begin{theorem}
...
\end{theorem}
The proof of Theorem \ref{thm:volume_ssd} is deferred to Appendix~\ref{sec:proof_of_volume_ssd}. We reiterate that the primary technical work involved in proving Theorem~\ref{thm:volume_ssd} involves showing that $\tau_{u}$ and $\phi_u$ are valid upper bounds on the normalized cut and mixing time; once this is established, the result follows more or less straightforwardly from~\citet{zhu2013}.
This result establishes that the volume of the symmetric set difference \smash{$\Delta(\Csig[\Xbf],
...
\begin{theorem}
  ...
  \end{equation}
  then the following statement holds: there exists a set $\Csig[\Xbf]^g \subseteq \Csig[\Xbf]$ of large volume 
  with $\vol(\Csig[\Xbf]^g; G_{n,r}) \geq \frac{1}{2}\vol(\Csig[\Xbf];G_{n,r})$ such that if Algorithm~\ref{alg: ppr} is run with any seed node $v \in \Csig[\Xbf]^g$,  then for any $n$ large enough so that
  \begin{equation*}
  n \geq c_1(\Pbb,r)\cdot (\log n)^{\max\{\frac{3}{d},1\}}
  \end{equation*}$\Cest$ satisfies~\eqref{eqn: consistent_density_cluster_recovery} with respect to $\Cset$ with probability at least $1 - \frac{c_2(\Pbb,r)}{n}$. Therefore, $\Cest$ is a consistent estimator of $\Cset$ in the sense of Definition \ref{def: consistent_density_cluster_estimation}. 
\end{theorem}
...
\begin{corollary}
	\label{cor: appr}
	Fix $\lambda > 0$, and let $\Cset \in \Cbb_f(\lambda)$ be a $\kappa$-well-conditioned cluster for some $\sigma > 0$. Choose input parameters $r, \alpha, v, (L,U)$ to be well-initialized in the sense of \eqref{eqn: initialization}, additionally set 
	\begin{equation}
...
\end{equation}
so that for any $\epsilon < \lambda < (1 - \epsilon)/2$, $\mathbb{C}_{f}(\lambda) = \set{\mathcal{C}_{\sigma}^{(1)}, \mathcal{C}_{\sigma}^{(2)}}$. Figure~\ref{fig:fig0} shows $f$ for two different choices of $\epsilon, \sigma$ and $\rho$. 

\begin{figure}[tb]
	\centering
	
	\includegraphics[width=0.4\textwidth]{lower_bound_plots/lower_bound_1}
	\includegraphics[width=0.4\textwidth]{lower_bound_plots/lower_bound_2}
	
	\caption{\it\small The density $f$ given by~\eqref{eqn:lb_density}, for two different choices of $\epsilon$ and $\sigma$. Left: $\epsilon = .3,\sigma = .1$; Right: $\epsilon = .2,\sigma = .2$. In both $\rho = 1$. } 
	\label{fig:fig0}
...
\textbf{Remark:}~~Technically, the rectangles $\Csig^{(1)}$ and $\Csig^{(2)}$ are not $\sigma$-expansions due to their sharp corners. To fix this, one can 
...
\textbf{Remark:}~~It is worth pointing out that the above conclusions are reliant on specific (albeit reasonable) ranges and choices of input parameters, which in some instances differ between the upper and lower bounds. We suspect that our lower bound continues to hold even when choosing input parameters as dictated by our upper bound; however, proving this is currently beyond our technical grasp, and is a matter for future work. 
...
\end{equation*}
Plugging in $\delta = \frac{1}{2}$, the theorem is satisfied by choosing constants $c_1 = 50$ and $c_3(\Pbb,r) = \frac{1}{4}(\cut_{\Pbb,r}(\Csig))^2$. 
...
\begin{proposition}
	...
	Under the setup and conditions of Theorem \ref{thm: mixing_time_upper_bound}, there exist constants $c_5(\Pbb,r)$,$c_6(\Pbb,r)$, and $c_7(\Pbb,r)$ independent of $n$ such that the following statement holds true: for any $n$ such that
	\begin{equation*}
	n \geq \max\left\{(\log n)^{\max\{\frac{3}{d},1\}}\left(\frac{1}{c_{5}(\Pbb,r)}\right)^d, c_{6}(\Pbb,r)\right\}
	\end{equation*}
	...
	\end{equation}
	hold with probability at least $1 - \frac{c_{7}(\Pbb,r)}{n} - 2n\exp\set{-\frac{2\lambda_{\sigma} \nu_d r^d n}{1875}} - 2\exp\set{-\frac{2}{25}n(\widetilde{\vol}_{\Pbb,r}(\Csig))^2}$.
\end{proposition}
...
\end{align*}
whereupon the theorem follows after an appropriate choice of universal constants $c_2 = 2^{34}, c_3 = 3$, and constants $c_{4}(\Pbb,r) := c_7(\Pbb,r) + \frac{7500}{\lambda_{\sigma}\nu_dr^d} + 25(\widetilde{\vol}_{\Pbb,r}(\Csig))^2$ and $c_1(\Pbb,r) = \frac{1}{c_5(\Pbb,r)^d} + c_6(\Pbb,r)$. In the rest of this section we proceed to proving Propositions \ref{prop: pointwise_mixing_time} and \ref{prop: local_spread_conductance}.
...
Roughly, our goal is to show that for sufficiently large $n$, with probability at least $1 - \frac{c_{7}(\Pbb,r)}{n}$
\begin{equation*}
...
\begin{lemma}
	...
	Under the setup and conditions of Theorem \ref{thm: mixing_time_upper_bound}, there exist constants $c_{5}(\Pbb,r)$ and $c_{7}(\Pbb,r)$ independent of $n$ such that the following statement holds true: for any $n$ such that
	\begin{equation*}
	\frac{n}{(\log n)^{\max\{\frac{3}{d},1\}}} \geq \left(\frac{1}{c_5(\Pbb,r)}\right)^d
	\end{equation*}
	...
	\end{equation}
	with probability at least $1 - \frac{c_{7}(\Pbb,r)}{n}$. 
\end{lemma}
...
\begin{lemma}
	...
	Let $T:\Csig \to \Csig[\Xbf]$ be a transportation map between $\widetilde{\Pbb}$ and $\widetilde{\Pbb}_n$. Suppose $\norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})} < \min \set{s(\widetilde{G}_{n,r}),r, \lambda_{\sigma}/(2^{d+1}d\Lambda_{\sigma}r)}$. Then there exists a constant $c_{9}(\Pbb,r) > 0$ which does not depend on the sample size $n$, such that for all $S \in \mathcal{L}(\widetilde{G}_{n,r})$, 
	\begin{equation}
	\label{eqn: graph_to_continuous_conductance}
	\widetilde{\Phi}_{n,r}(S) \geq \widetilde{\Phi}_{\Pbb,r}(T^{-1}(S)) - \frac{c_{9}(\Pbb,r)\norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})} }{s(\widetilde{G}_{n,r}) - c_{9}(\Pbb,r) \norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})}  }
	\end{equation}
\end{lemma}
...
\begin{theorem}[Restatement of Proposition 5 of \cite{garciatrillos16}]
	...
	There exists constants $c_5(\Pbb,r)$ and $c_{7}(\Pbb,r)$ which do not depend on the sample size such that for any $\delta > 0$, the following statement holds: for any $n$ such that
	\begin{equation*}
	\frac{n}{(\log n)^{\max\{\frac{3}{d},1\}}} \geq \left(\frac{1}{c_5(\Pbb,r)\delta}\right)^{d}
	\end{equation*}
	then with probability at least $1 - \frac{c_{7}(\Pbb,r)}{n}$ there exists a transportation map $T_n$ between $\widetilde{\Pbb}$ and $\widetilde{\Pbb}_n$
	which satisfies
	\begin{equation*}
...
\begin{align*}
\min_{S \in \mathcal{L}(\widetilde{G}_{n,r})} \widetilde{\Phi}_{n,r}(S) & \geq \min_{S \in \mathcal{L}(\widetilde{G}_{n,r})} \widetilde{\Phi}_{\Pbb,r}(T_n^{-1}(S)) - \frac{c_{9}(\Pbb,r) \norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})} }{s(\widetilde{G}_{n,r}) - c_{9}(\Pbb,r)\norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})}} \\
& \geq  \widetilde{\Phi}_{\Pbb,r} -  \frac{c_{9}(\Pbb,r) \norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})} }{s(\widetilde{G}_{n,r}) - c_{9}(\Pbb,r)\norm{\mathrm{Id} - T}_{L^{\infty}(\widetilde{\Pbb})}} \\
& \geq \frac{\widetilde{\Phi}_{\Pbb,r}}{2},
\end{align*}
where the last inequality follows since $\delta < \frac{\widetilde{\Phi}_{\Pbb,r}\cdot s(\widetilde{G}_{n,r})}{2c_{9}(\Pbb,r) + \widetilde{\Phi}_{\Pbb,r}}$. Then, Lemma~\ref{lem: nonuniform_continuous_conductance} yields the desired result. It remains to prove Lemmas \ref{lem: graph_to_continuous_conductance} and \ref{lem: nonuniform_continuous_conductance}, which we do in the following two subsections.
...
\begin{equation*}
\frac{\widetilde{\cut}_{n,r}(S,S^c)}{\widetilde{\vol}_{n,r}(S)} \geq \widetilde{\Phi}_{\Pbb,r}(\Sset) - \frac{c_{9}(\Pbb,r)\varDelta}{s(\widetilde{G}_{n,r}) - c_{9}(\Pbb,r) \varDelta}
\end{equation*}
for some constant $c_{9}(\Pbb,r) > 0$. We first state the relationships between the discrete functionals $\widetilde{\cut}_{n,r}$ and $\widetilde{\vol}_{n,r}$, and the continuous functionals $\widetilde{Q}_{\Pbb,r}$ and $\widetilde{\pi}_{\Pbb,r}$, alluded to in the previous section.
\begin{lemma}
...
\begin{lemma}
	...
	Suppose $\Delta \leq r$. Then there exist constants $c_{10}(\Pbb,r),c_{11}(\Pbb,r) \geq 0$ which do not depend on the sample size $n$, such that for every $\Sset \subseteq \Csig$,
	\begin{align*}
	\int_{\Csig} \ell_{\Pbb,r^+}(x) \,d\Pbb(x) & \leq \int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x) + c_{10}(\Pbb,r) \varDelta \\
	\int_{\Csig} \ell_{\Pbb,r^-}(x) \,d\Pbb(x) & \geq \int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x) - c_{10}(\Pbb,r) \varDelta \\
	\widetilde{Q}_{\Pbb,r^-}(\Sset, \Sset^c) & \geq \widetilde{Q}_{\Pbb,r}(\Sset, \Sset^c) - c_{11}(\Pbb,r) \varDelta \\
	\widetilde{\pi}_{\Pbb,r^+}(\Sset) & \leq \widetilde{\pi}_{\Pbb,r}(\Sset) + c_{11}(\Pbb,r) \varDelta.
	\end{align*}
\end{lemma}
...
& \geq \frac{\widetilde{Q}_{\Pbb,r}(\Sset,\Sset^c) - c_{11}(\Pbb,r) \varDelta }{\widetilde{\pi}_{\Pbb,r^+}(\Sset) + c_{11}(\Pbb,r) \varDelta} \left(1 - \frac{2 c_{10}(\Pbb,r) \varDelta}{\int_{\Csig}\ell_{\Pbb,r}(x)\,d\Pbb(x)}\right) \tag{Lemma \ref{lem: deviation_transportation_distance} } \\
& \geq \frac{\widetilde{Q}_{\Pbb,r}(\Sset,\Sset^c)}{\widetilde{\pi}_{\Pbb,r}(\Sset)} - \left(\frac{2c_{11}(\Pbb,r)}{\widetilde{\pi}_{\Pbb,r}(\Sset)} + \frac{2c_{10}(\Pbb,r)}{\int_{\Csig}\ell_{\Pbb,r}(x)\,d\Pbb(x)}\right)\varDelta \label{eqn: proof_of_lemma_graph_to_continuous_1}
\end{align}
...
\begin{lemma}
	...
	Suppose $\varDelta \leq \lambda_{\sigma}/(2^{d+1}d\Lambda_{\sigma}r)$. Then there exists a constant $c_{12}(\Pbb,r) > 0$ which does not depend on the sample size $n$ such that for every $S \subseteq \Csig[\Xbf]$,
	\begin{equation*}
	\widetilde{\pi}_{\Pbb,r}(\Sset) \geq \widetilde{\pi}_{n,r}(S) - c_{12}(\Pbb,r) \varDelta
	\end{equation*}
\end{lemma}
Combining this result with~\eqref{eqn: proof_of_lemma_graph_to_continuous_1}, we obtain
\begin{align*}
\frac{\widetilde{\cut}_{n,r}(S,S^c)}{\widetilde{\vol}_{n,r}(S)} & \geq \widetilde{\Phi}_{\Pbb,r}(\Sset) - \left(\frac{2c_{11}(\Pbb,r)}{\widetilde{\pi}_{n,r}(S) - c_{12}(\Pbb,r)\varDelta} + \frac{2c_{10}(\Pbb,r)}{\int_{\Csig}\ell_{\Pbb,r}(x)\,d\Pbb(x)}\right)\varDelta \\
& \geq \widetilde{\Phi}_{\Pbb,r}(\Sset) - \left(\frac{2c_{11}(\Pbb,r)}{s(\widetilde{G}_{n,r}) - c_{12}(\Pbb,r)\varDelta} + \frac{2c_{10}(\Pbb,r)}{\int_{\Csig}\ell_{\Pbb,r}(x)\,d\Pbb(x)}\right)\varDelta
\end{align*}
where the latter inequality follows as we assumed $S \in \mathcal{L}(\widetilde{G}_{n,r})$. Choosing the constant $c_{9}(\Pbb,r)$ in Lemma \ref{lem: graph_to_continuous_conductance} to be $c_{9}(\Pbb,r) = \max\set{2c_{10}(\Pbb,r)/\bigl(\int_{\Csig}\ell_{\Pbb,r}(x)\,d\Pbb(x) \bigr) + 2c_{11}(\Pbb,r), c_{12}(\Pbb,r)}f$, we achieve our desired result. It remains to show Lemmas~\ref{lem: cut_volume}, \ref{lem: deviation_transportation_distance}, and \ref{lem: continuous_pi_lb}, which we now turn to.
...
& \leq \Pbb(\Csig) \Lambda_{\sigma} \nu_d  2^{d}d r^{d-1}\varDelta =: c_{10}(\Pbb,r) \varDelta. \label{eqn: deviation_transportation_distance_1}
\end{align}
...
& \leq \int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x) + c_{10}(\Pbb,r)\varDelta.
\end{align*}
...
& \geq \widetilde{Q}_{\Pbb,r}(\Sset, \Sset^c) - \frac{c_{10}(\Pbb,r) \varDelta}{\int_{\Csig}\ell_{\Pbb,r}(x) \,d\Pbb(x)} =:\widetilde{Q}_{\Pbb,r}(\Sset, \Sset^c) - c_{11}(\Pbb,r) \varDelta.
\end{align*}
...
& \leq \widetilde{\pi}_{\Pbb,r}(\Sset) + c_{11}(\Pbb,r)\varDelta.
\end{align*}
...
& \leq \frac{\int_{\Sset} \ell_{\Pbb,r}(x) \,d\Pbb(x) + c_{10}(\Pbb,r)\varDelta}{\int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x) - c_{10}(\Pbb,r)\varDelta} \\
& \leq \widetilde{\pi}_{\Pbb,r}(\Sset) + \frac{2c_{10}(\Pbb,r)\varDelta}{\int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x) - c_{10}(\Pbb,r)\varDelta} \\
& \leq \widetilde{\pi}_{\Pbb,r}(\Sset) + \frac{4c_{10}(\Pbb,r)\varDelta}{\int_{\Csig} \ell_{\Pbb,r}(x) \,d\Pbb(x)} =: \widetilde{\pi}_{\Pbb,r}(\Sset) + c_{12}(\Pbb,r) \varDelta.
\end{align*}
where the last inequality follows from the assumption $\varDelta \leq \lambda_{\sigma}/(2^{d + 1} dr\Lambda_{\sigma})$, which implies $c_{10}(\Pbb,r)\varDelta \leq \int_{\Csig}\ell_{\Pbb,r}(x) \,d\Pbb(x) / 2$.
...
\end{equation}
with probability at least $1 - 2\exp\{-\frac{1}{4} n \vol_{\Pbb,r}(\Csig)^2\}$. Therefore, conditional on \eqref{eqn:volume_ssd_pf2} the cluster estimate $\Cest = S_{\beta,v}$ for some $\beta \in \frac{1}{\vol_{n,r}(\Csig[\Xbf])}\left(\frac{1}{100},\frac{2}{5}\right)$, and therefore the upper bound \eqref{eqn:volume_ssd_pf1} holds with respect to $\vol_{n,r}(\widehat{C} \vartriangle \Csig[\Xbf])$. To summarize, we have shown that when
\begin{equation*}
n \geq c_1(\Pbb,r)(\log(n))^{\max\{\frac{3}{d},1\}}
\end{equation*}
then
\begin{equation*}
\vol_{n,r}(\Csig[\Xbf] \vartriangle \widehat{C}) \leq  2180 \cdot\kappa(\Cset)\cdot\vol_{n,r}(\Csig[\Xbf])
\end{equation*} 
with probability at least $1 - \frac{c_4(\Pbb,r)}{n} - 3\exp\set{-(c_3(\Pbb,r) + \frac{1}{4}\vol_{\Pbb,r}(\Csig)^2)n}$. Since $c_3(\Pbb,r) \leq \vol_{\Pbb,r}(\Csig)^2$, letting $c_2(\Pbb,r) := c_4(\Pbb,r) + \frac{6}{c_3(\Pbb,r)}$ we have proved Theorem~\ref{thm:volume_ssd}.
...
\end{equation*}
holds with probability at least $1 - \frac{c_4(\Pbb,r)}{n}$.
...
\begin{itemize}
	...
	\end{equation*}
	with probability at least $1 - 3\exp\{-nc_1(\Pbb,r)\}$. By \eqref{eqn: initialization}, $\alpha \geq 1/(10 \taubf(\theta))$ and therefore $2 \Phi(\Csig[\Xbf]; G_{n,r})/\alpha \leq 20 \kappa(\Cset)$.
...
\end{align}
where the last inequality again follows by choosing $\delta = 1/2$ and \eqref{eqn: kappa_ub}. We have shown~\eqref{eqn: PPR_gap} and therefore proved Theorem~\ref{thm: consistent_recovery_of_density_clusters} (choosing $c_2(\Pbb,r) = c_4(\Pbb,r) + \frac{6}{c_3(\Pbb,r)} + \frac{900}{\lambda_{\sigma}\nu_dr^d} + \frac{1}{4}\vol_{\Pbb,r}(\Csig)^2$).
