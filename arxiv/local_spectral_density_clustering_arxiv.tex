...
Finally, we employ the sweep cut method to obtain a local cluster from the PPR embedding $\pbf_v$. For a level $\beta > 0$ and a target stationary probability $\pi_0 > 0$, we define a \emph{$\beta$-sweep cut} of $\pbf_v = (\pbf_v(u))_{u \in V}$
as   
\begin{equation}
\label{eqn: sweep_cuts}
S_\beta := \set{u \in V: \pbf_v(u) > \beta \pi_0}.
\end{equation}
...
(\frac{1}{40},\frac{1}{12})$}\footnote{The choice of a specific range such as  
\smash{$(\frac{1}{40}, \frac{1}{12})$} is standard in the analysis of PPR
...
\begin{algorithmic}[1]
  ...
  \STATE Compute the PPR vector $\pbf_v=\pbf(v, \alpha; G_{n,r})$ as in \eqref{eqn: 
    ppr_vector}. 
  \STATE For \smash{$\beta \in (\frac{1}{40}, \frac{1}{12})$} compute sweep cuts 
...
A substantial portion of our technical effort involves estimating graph-theoretic connectivity properties of empirical density clusters in the neighborhood graph $G_{n,r}$. In particular, we prove an upper bound on the mixing time of a random walk run only over the subset of nodes in $G_{n,r}$ which fall within the empirical density cluster $\Cset[\Xbf]$. To do so, we rely on a series of seminal works upper bounding the mixing time of \emph{geometric random walks} (see \citet{vempala2005} for a comprehensive review.)  \citet{dyer1991} initiated this study, using these random walks as a fundamental subroutine to efficiently compute volumes of high-dimensional convex bodies. These results are improved in \citep{lovasz1990,kannan97,kannan06}, who show, among other things, that the bounds on mixing time can be sharpened by avoiding so-called ``start-penalties''.(These improvements are crucial to our work, for they allow us to avoid factors of $\log n$ which would imply divergence as $n \to \infty$.) Following the lead of \citep{abbasi-yadkori2016,abbasi-yadkori2016a}, we extend these results to hold for Lipschitz deformations of convex sets. Additionally, we relate the mixing time of these (continuous-space) geometric random walks to the mixing time of random walks over (discrete) neighborhood graphs.

Finally, it is worth mentioning that density clustering and level set estimation is of course a well-studied problem. \citet{polonik1995, rigollet2009} study density clustering under the 
...
\item In Section \ref{sec: examples}, we show the implications of our theory for some simple generative models, and compare our upper bounds on the symmetric set difference metric to those achievable by classical density cluster estimators.
...
\textcolor{red}{Mention Arias-Castro and Little here.}
...
well-conditioned density cluster $\Cset$ (with respect to some $\theta$), we require that the tuning parameters of Algorithm~\ref{alg: ppr} are chosen to fall within the ranges
\begin{equation}
\begin{gathered}
...
\pi_0 \in [3/4,5/4] \cdot \frac{\lambda_{\sigma}}{n\Lambda_{\sigma}\Pbb(\Csig)},
\end{gathered}
\end{equation}
...
We now present our first main result: a bound on the symmetric set difference metric between the estimated cluster $\widehat{C}$ and empirical cluster $\Csig[\Xbf]$.
\begin{theorem}
  \label{thm: misclassification_rate}
  Fix $\lambda > 0$ and let \smash{$\Cset \in \Cbb_f(\lambda)$} be a
  $\kappa$-well-conditioned density cluster (with respect to some $\theta$). If Algorithm \ref{alg: ppr} is well-initialized, there exists a universal constant $c > 0$ such that with probability tending to 1 as $n \to \infty$,  
  \begin{equation}
    \label{eqn: misclassification_rate_ub}
    \Delta(\Csig[\Xbf], \Cest) \leq c \kappa(\Cset)  \frac{\Lambda_{\sigma}}{\lambda_{\sigma}} \abs{\Csig[\Xbf]}
  \end{equation}
\end{theorem}
...
The bound on symmetric set difference \eqref{eqn: misclassification_rate_ub} does not imply consistent density cluster estimation in the sense of \eqref{eqn: consistent_density_cluster_recovery}. This notion of
consistency requires a uniform bound over $\pbf$: for all \smash{$\Cset'
  \in \Cbb_f(\lambda), \Cset' \neq \Cset$}, and each $u \in \Cset, w \in
\Cset'$,  
\begin{equation}
\label{eqn: ppr_gap}
\pbf_v(w) \leq \frac{\pi_0}{40} < \frac{\pi_0}{11} \leq \pbf_v(u),
\end{equation}
so that any sweep cut $S_{\beta}$ for $\beta \in [1/40,1/11]$ (i.e., any
...
small upper bound on $\kappa(\Cset)$ ensures that as $n \to \infty$, such a gap exists with probability
1, and hence guarantees \smash{$\Cest$} will be a consistent  
...
We illustrate the implications of our theory on some common generative models for nonparametric clustering, and compare our theoretical bounds on PPR to those derived for more classical density cluster estimators.

\subsection{Uniform clusters plus noise, dimension $d = 1$.}
\label{subsec: uniform_clusters}

We begin with a very simple model $\Pbb$ that fits ideally into our framework. For $i = 1,\ldots,n$, let $z_i \sim \delta \textrm{Bern}(1/2)$ be a latent class assignment, and let $w_i \sim \sigma \textrm{Unif}([-\sigma, \sigma])$ where $\delta$ and $\sigma$ are positive numbers such that $\delta > 3 \sigma$ (to ensure adequate separation of the density clusters). Assume we sample $x_1, \ldots, x_n$ from the following contaminated mixture model: 
\begin{equation*}
x_i = 
\begin{cases}
z_i + w_i,& \quad \textrm{with probability $1 - \epsilon$} \\
\widetilde{w}_i, & \quad \textrm{with probability $\epsilon$}
\end{cases}
\end{equation*}
where $\widetilde{w}_i \sim \textrm{Unif}(\sigma, \delta - \sigma)$ and $\epsilon$ is a positive number between $0$ and $1$. In other words, $\Pbb = (1-\epsilon)\bigl((1/2)\textrm{Unif}[-\sigma,\sigma] + (1/2)\textrm{Unif}[\delta - \sigma,\delta + \sigma]\bigr) + \epsilon \textrm{Unif}(\sigma,\delta - \sigma)$ is a mixture of two uniforms with added background noise. We will consider $\Cset = \set{0}$, so that the $\sigma$-expansion of $\Cset$ is $\Csig = [-\sigma,\sigma]$.\footnote{Although $\Cset$ is not a density level set of $\Pbb$, one can add a small bump to the density around $0$ to make $\set{0}$ a well defined density cluster for an appropriate choice of threshold $\lambda$, without changing any of the subsequent calculations. For ease of presentation, we ignore this detail.}

We verify that each of the assumptions \ref{asmp: bounded_density}-\ref{asmp: bounded_volume} hold on $\Csig$. The maximum and minimum density over $\Csig$ are the same, $\Lambda_{\sigma} = \lambda_{\sigma} = (1 - \epsilon)/(4\sigma)$. Letting $\Csig' = [\delta - \sigma, \delta + \sigma]$ be the $\sigma$-expansion around $\Cset' = \set{\delta}$, the condition $\dist(\Csig, \Csig') > \sigma$ holds since $\delta > 3\sigma$. The low noise density assumption holds with parameter $\gamma = 0$ and $c_0 = (1 - \epsilon)/(4\sigma) - \epsilon/2\sigma$. The set $\Csig$ is convex with diameter $\rho = 2\sigma$. Finally, for any $0 < r < \sigma$ the bounded volume condition of \ref{asmp: bounded_volume} holds. 

We can now give an upper bound on misclassification error that holds with probability $1$ as $n \to \infty$. To do so, we begin by computing $\Phibf(\theta)$ and $\taubf(\theta)$ as defined in \eqref{eqn: condition_number_1},
\begin{equation*}
\Phibf(\theta) = \frac{c_1 r \epsilon}{(\delta - 2\sigma)(1 - \epsilon)}, \quad \taubf(\theta) = 4 c_2 \frac{\sigma^2}{r^2} \log\left(\frac{4c_4\sigma}{(1 - \epsilon)r}\right) + c_3.
\end{equation*}
Therefore, if we set $r = \sigma/2$, the largest value permissible under the conditions of Theorems \ref{thm: conductance_upper_bound} and \ref{thm: mixing_time_upper_bound}, we obtain the condition number
\begin{equation*}
\kappa(\Cset) = \frac{4c_1c_2\sigma\epsilon}{(\delta - 2\sigma)(1 - \epsilon)}\log^2\left(\frac{8c_4}{(1 - \epsilon)}\right) + \frac{c_3c_1\epsilon\sigma}{2(\delta - 2\sigma)}
\end{equation*}

Therefore, by Theorem \ref{thm: misclassification_rate}, if Algorithm \ref{alg: ppr} is well-initialized with respect to $\Cset$, then we have that the fraction of misclassified points $\frac{\Delta(\Cest, \Csig[\Xbf])}{\abs{\Csig[\Xbf]}}$ is roughly upper bounded by $\sigma \epsilon / (\delta - 2\sigma)$ (ignoring constants and logarithmic terms). Qualitatively speaking, this exhibits the correct dependence on the mixture model parameters $\sigma$, $\epsilon$, and $\delta$. In particular, as the expected fraction of background noise $\epsilon$, or the ratio of cluster width to cluster separation $\sigma/(\delta - 2\sigma)$, decrease, so does our estimate of the symmetric set difference $\Delta(\Cest, \Csig[\Xbf])$.

\subsection{Uniform clusters plus noise, general dimension.}

To better demonstrate the flexibility of our theory, we now generalize the previous example in several respects. For an integer $d' < d$, consider the $d'$-dimensional unit cube embedded in $\Reals^d$ by letting
\begin{equation*}
[0,1]^{d'} = \{(x_1,\ldots,x_d) \in \Rd: 0 \leq x_i \leq 1 ~\textrm{for}~ i = 1,\ldots,d', x_i = 0 ~\textrm{for}~ i = d'+ 1,\ldots,d\},
\end{equation*}
and denoting the $\sigma$-expansion of $[0,1]^{d'}$ by $[0,1]_{\sigma}^{d'} = \{x \in \Rd: \dist(x,[0,1]^{d'}) < \sigma\}$. For a number of clusters $K > 2$, let $f_1,\ldots,f_K: [0,1]_{\sigma}^{d'} \to [0,1]^{d}$ be Lipschitz, measure-preserving maps, with Lipschitz constant $\norm{f_k}_{\textrm{Lip}}, ||f_k^{-1}||_{\textrm{Lip}} \leq L$ for each $k$. Write $\Csig^k$ for the image $f_k([0,1]_{\sigma}^{d'})$, and let $\Psi_k$ be the uniform distribution over $\Csig^k$. Additionally, let $\Psi_0$ be the uniform distribution over $\Cset^0 = [0,1]^d \setminus \bigcup_{k = 1}^{K} \Csig^k$. For $\epsilon \in [0,1)$, let $\Pbb$ be the $K$-component mixture model given by
\begin{equation*}
\Pbb = \frac{(1 - \epsilon)}{K} \sum_{k = 1}^{K} \Psi_k + \epsilon \Psi_0. 
\end{equation*}

Suppose we observe $x_1,\ldots,x_n$ sampled i.i.d from $\Pbb$. Letting $f$ be the density function of $\Pbb$, for sufficiently small $\epsilon > 0$, and an appropriate choice of $\lambda$, the density upper level set $\Cbb_f(\lambda)$ will contain components $\Csig^1,\ldots,\Csig^k$. Let's say the set we are interested in recovering is $\Csig^1$. Under certain regularity conditions, this framework fits smoothly into the theory developed Sections~\ref{sec: consistent_cluster_estimation_with_ppr} and \ref{sec: analysis}. We will assume that for each $k$, $\Csig^k$ is a $\sigma/2$-expansion\footnote{Note that it is possible that $\Csig^k = f_k([0,1]_{\sigma}^{d'})$ for a Lipschitz function $f_k$, and yet itself not be an $\sigma/2$-expansion of any set. As we need $\Csig^k$ to be a $\sigma$-expansion, we make this requirement explicit.} of some $\Cset^k \subset [0,1]^d$ and additionally that
\begin{equation*}
\min_{k \neq \ell} \dist(\Csig^k,\Csig^\ell) > \sigma, \min_{k}\dist(\Csig^k,\partial [0,1]^d) > \sigma, \quad \textrm{and}~ 5 < K < \frac{\sigma^{d - d'}}{2^{d' + 1}}.
\end{equation*}

Now, we verify that each of the assumptions \ref{asmp: bounded_density}-\ref{asmp: bounded_volume} hold with respect to $\Csig^1$. 
\begin{itemize}
	\item \ref{asmp: bounded_density}. In this case, the minimum and maximum density within the $\sigma/2$-expansion $\Csig^1$ are the same, $\Lambda_{\sigma} = \lambda_{\sigma} = (1 - \epsilon)/(K \nu(\Csig^1))$. (Indeed, this holds for each $\Csig^k$.) 
	\item \ref{asmp: cluster_separation}. By assumption, $\dist(\Csig^1,\Csig^k) > \sigma$ for each $k = 2,\ldots,K$. 
	\item \ref{asmp: low_noise_density}. The low noise condition is satisfied with $\gamma = 0$ and $c_0 = (1 - \epsilon)/(K \nu(\Csig^1)) - \epsilon/(\nu(\Cset^0))$. 
	\item \ref{asmp: embedding} By assumption, the embedding condition is satisfied with $\rho = 1 + \sigma < 2$, and $L = L$. 
	\item \ref{asmp: bounded_volume}. When $r < \sigma/(2d)$ we have that $\nu( B(x,r) \cap \Csig^k)\geq \frac{6}{25}\lambda_{\sigma}\nu_d r^d$ (see Lemma~\ref{lem: local_conductance}). Therefore
	\begin{align*}
	\sum_{k = 2}^{K} \int_{\Csig^k} \Pbb(B(x,r)) \,dx & \geq \frac{6}{25}\sum_{k = 2}^{K}\nu(\Csig^k) \lambda_{\sigma} \nu_d r^d \\
	& \overset{(i)}{\geq} \nu(\Csig^1) \lambda_{\sigma} \nu_d r^d \\
	&  \geq \int_{\Csig^1} \Pbb(B(x,r)) \,dx.
	\end{align*} 
	where $(i)$ follows from the fact that $\nu(\Csig^k) = \nu([0,1]_{\sigma}^{d'})$ for all $k$ and the assumption that $K \geq 5$. Therefore \ref{asmp: bounded_volume} is satisfied.
\end{itemize}

The restriction that $f$ be measure-preserving implies $\nu(\Csig^k) = \nu([0,1]_{\sigma}^{d'}) \leq 2^{d'} \sigma^{d - d'}$ for each $k$. Along with the upper bound on number of clusters $K$, we additionally have $\nu(\Cset^0) \geq (1 - K 2^{d'} \sigma^{d - d'}) \geq 1/2$. We can therefore bound $\Phibf(\theta)$ and $\taubf(\theta)$,
\begin{equation}
\label{eqn:normalized_cut_mixing_time_ub_example2}
\Phibf(\theta) \leq c \frac{K dr \epsilon}{\sigma 2^{d'} \sigma^{d - d'} (1 - \epsilon)}, \quad \Psibf(\theta) \leq c \frac{d^3 L^2}{r^2} \log^2\left(\frac{K \sigma^{d - d'}}{(1 - \epsilon)r}\right),
\end{equation}
and letting $r = \sigma/(2d)$, we obtain the following bound on the condition number $\kappa(\Cset)$:
\begin{equation}
\label{eqn:kappa_example2}
\kappa(\Cset) \leq c \frac{\epsilon \sigma^{d - d' - 2} K d^5 L^2}{(1 - \epsilon)} \log^2\left(\frac{d K \sigma^{d - d'}}{\sigma(1 - \epsilon)}\right),
\end{equation}
which likewise implies a bound on the symmetric set difference metric $\Delta(\Cest,\Csig[\Xbf])$ (assuming that Algorithm~\ref{alg: ppr} is well-initialized.)

\subsubsection{Plug-in density clustering.}
It is interesting to compare \eqref{eqn:kappa_example2} to the symmetric set difference obtained by a more classical density cluster estimator. In particular, we consider the plugin approach to density cluster estimation, which we now briefly summarize (see e.g. \cite{chaudhuri2010} for a more formal algorithmic description). To motivate this approach, note that one can view the degrees $\deg_{n,r}(x_1), \ldots, \deg_{n,r}(x_n)$ in the graph $G_{n,r}$ as estimates of the density evaluated at data $f(x_1),\ldots,f(x_n)$. Therefore, a natural way to identify points in high-density regions is to throw out points without sufficiently high degree. To identify the high-density regions $\Csig^1,\ldots,\Csig^k$, consider the threshold
\begin{equation*}
\lambda' = \frac{\epsilon}{\nu(\Cset^0)} + \frac{c_0}{2}.
\end{equation*}
For any $x_i \in \bigcup_{k = 1}^{K}\Csig^k[\Xbf]$ such that $\dist(x_i, \Cset^0) > r$, the expected degree of $x_i$ is lower bounded $\Ebb(\deg_{n,r}(x_i)) = \lambda_{\sigma} \nu_d r^d = (\lambda' + \frac{c_0}{2})\nu_dr^d$. On the other hand, for any $x_i \in \Cset^0[\Xbf]$ such that $\dist(x_i, \bigcup_{k = 1}^{K}\Csig^k[\Xbf]) > r$, the expected degree of $x_i$ is upper bounded $\Ebb(\deg_{n,r}(x_i)) \leq (\lambda' - \frac{c_0}{2})\nu_dr^d$. Let $n$ be large enough to allow for concentration of the degrees around their means. Then the set $C_n(\lambda') = \set{x_i \in \Xbf: \deg_{n,r}(x_i) > \lambda'\nu_dr^d}$ serves as a reasonable estimate of $\bigcup_{k = 1}^{K} \Cset_{\sigma}^k[\Xbf]$, in the sense that with high probability $\bigcup_{k = 1}^{K} \Cset_{\sigma - r}^k[\Xbf] \subseteq C_n(\lambda') \subseteq \bigcup_{k = 1}^{K} \Cset_{\sigma + r}^k[\Xbf]$  

For a sufficiently large choice of radius $r$, we can draw the additional conclusion that the subsets $\Cset_{\sigma - r}^k[\Xbf]$ are connected in $G_{n,r}$, and therefore in the graph $G_{n,r}[C_n(\lambda')]$ as well. This implies that there are connected components $C^1, \ldots, C^K$ of $G_{n,r}[C_n(\lambda')]$ which are good estimates of $\Csig^1[\Xbf],\ldots,\Csig^K[\Xbf]$, since with high probability $\Cset_{\sigma - r}^k[\Xbf] \subseteq C^k \subseteq \Cset_{\sigma + r}^k[\Xbf]$. 

Those points $x_i \in \Xbf$ which are in $\Cset_{\sigma + r}^k$ but not $\Cset_{\sigma}^k$ are those points for which $\Ebb(\deg_{n,r}(x_i))$ may be  approximately equal to $\lambda'$, and so we cannot rule out their belonging to $C^k$. The symmetric set difference $\Delta(C^k, \Csig^k[\Xbf])$ is therefore roughly equal to the cardinality of $\{x_i \in X: 0 < \dist(x_i,\Csig^k) \leq r\}$. 

To compare this to \eqref{eqn:kappa_example2}, we must provide an estimate of the cardinality of $\{x_i \in X: 0 < \dist(x_i,\Csig^k) \leq r\}$. By Lemma~\ref{lem: expected_number_boundary_points}, we have the following upper bound on the expected number of points in this set,
\begin{equation*}
\Ebb \abs{\{x_i \in X: 0 < \dist(x_i,\Csig^k) \leq r\}} \leq \frac{4ndr}{\sigma}\left(\lambda_{\sigma} - c_0\right)\nu(\Csig).
\end{equation*}
Combined with the fact that $\Ebb \abs{\Csig^k[\Xbf]} = n \lambda_{\sigma} \nu(\Csig)$--and using our bounds on $\lambda_{\sigma}$ and $c_0$ from the previous section--we obtain
\begin{equation*}
\frac{\Ebb\abs{\{x_i \in X: 0 < \dist(x_i,\Csig^k) \leq r\}} }{\Ebb \abs{\Csig^k[\Xbf]}} \leq c \frac{K dr \epsilon}{\sigma 2^{d'} \sigma^{d - d'} (1 - \epsilon)},
\end{equation*}
where we recognize the right hand side as being identical to the bound on normalized cut we derived in \eqref{eqn:normalized_cut_mixing_time_ub_example2}.

Let us summarize our findings with respect to the particular density cluster $\Csig^1$. First, with high probability, only those points $x_i \in X$  such that $0 < \dist(x_i,\Csig^1) \leq r$ will be misclassified by the plug in approach. Second, the expected ratio of the number of such points, to the number of total data points that fall in $\Csig^1$, is upper bounded by the same upper bound that we derived on the normalized cut $\Phi_{n,r}(\Csig^1[\Xbf];G_{n,r})$. In other words, for a particular choice of radius $r$, the symmetric set difference $\Delta(C^1,\Csig^1[\Xbf])/\abs{\Csig^1[\Xbf]}$ is comparable to the normalized cut of $\Csig^1[\Xbf]$ in $G_{n,r}$, whereas the symmetric set difference $\Delta(C^1,\Csig^1[\Xbf])/\abs{\Csig^1[\Xbf]}$ is upper bounded by the \emph{product} of the normalized cut and the mixing time, which is a looser bound. This comparison elucidates an important distinction between estimation of density clusters via PPR, and more classical methods based on plug in estimators. The latter approach is robust to poor internal connectivity of the $\sigma$-expansion $\Csig$, whereas the former may not be. 
...
\begin{lemma}
	...
	(1 - \epsilon)\Pbb(\Csig) & \leq \frac{\abs{\Csig[\Xbf]}}{n} \leq (1 + \epsilon)\Pbb(\Csig) \label{eqn: Csig_cardinality_bound}
	\end{align}
	...
	If additionally \ref{asmp: C'_bounded_density} holds, then for any $\epsilon > 0$, the following bound holds with probability tending to one as $n \to \infty$:
	\begin{equation}
...
\begin{proof}
	...
	Now, for any $\epsilon > 0$, letting $\mu = \int_{\Csig} \Pbb(B(x,r)) \,d\Pbb(x) = \Ebb\left(\frac{\widetilde{\vol}_{n,r}(\Csig[\Xbf])}{n(n-1)}\right)$ we have
	\begin{equation*}
...
In this section, we prove Theorem \ref{thm: misclassification_rate}.
The result is in essence a consequence of i) a result of \citet{zhu2013} relating the quality of the PPR output to normalized cut and mixing time, and ii) the upper bounds in Theorem~\ref{thm: conductance_upper_bound} 
and Theorem~\ref{thm: mixing_time_upper_bound} on these functionals. 

We begin with Lemma \ref{lem: zhu}, which is a restatement of Corollary 3.3 of \citet{zhu2013}, translated into notation consistent with the rest of this paper. It is stated with respect to an arbitrary $S \subseteq G = (V,E)$, and a good set $S^g \subseteq S$ with $\vol(S^g; G) \geq \vol(S; G)/2$.

\begin{lemma}
	\label{lem: zhu}
	Let $\varepsilon \geq 0$, and suppose $v \in S^g$ and $\alpha \leq \frac{1}{9\tau_{\infty}(G[S])}$. Consider the approximate PPR vector $p^{(\varepsilon)}_v := p^{(\varepsilon)}(v,\alpha;G)$. Then,
	\begin{equation}
	\label{eqn:zhu_1}
	p^{(\varepsilon)}_v(V \setminus S) \leq 2\frac{\Phi(S;G)}{\alpha},
	\end{equation}
	and additionally there exists a residual vector $p_{\ell} \in [0,1]^V$ with $\norm{p_{\ell}}_1 \leq 2\Phi(S;G)/\alpha$ such that
	\begin{equation}
	\label{eqn:zhu_2}
	\textrm{for all $u \in S$}, \quad p^{(\varepsilon)}_v(u) \geq \frac{4}{5} \frac{\deg(u;G[S])}{\vol(S;G)} - \varepsilon \deg(u;G) - p_{\ell}(u).
	\end{equation}
\end{lemma}

\noindent Now, consider the sweep cut set
\begin{equation*}
S_{\beta} := \set{u \in V: p^{(\varepsilon)} \geq \beta \pi_0},
\end{equation*}
and suppose the conditions (on seed node $v$ and teleportation parameter $\alpha$) of Lemma~\ref{lem: zhu} are met. Some straightforward calculations show the implications of Lemma~\ref{lem: zhu} on the symmetric set difference metric $\Delta(S_{\beta},S)$. In particular,
\begin{equation}
\abs{S_{\beta} \setminus S} \leq \frac{p_v(V \setminus S; G)}{\min_{u \in S_{\beta}}p^{(\varepsilon)}_v(u)} \leq \frac{2 \Phi(S;G)/\alpha}{\beta \pi_0}. \label{eqn:zhu_3}
\end{equation}
To upper bound $\abs{S \setminus S_{\beta}}$, we first observe that by definition, for every node $u \in S \setminus S_{\beta}$ $p^{(\varepsilon)}(u) \leq \beta \pi_0$. Therefore by \eqref{eqn:zhu_2} we have that the residual vector is lower bounded 
\begin{equation}
\label{eqn:zhu_5}
p_{\ell}(u) \geq \frac{4\deg(u;G[S])}{5\vol(S;G)} - \varepsilon \deg(u;G) - \beta\pi_0  = \underbrace{\left(\frac{4}{5} - \varepsilon \vol(S;G) \frac{\deg(u;G)}{\deg(u;G[S])}\right)}_{T_1(u,\varepsilon)}\frac{\deg(u;G[S])}{\vol(S;G)} - \beta\pi_0
\end{equation}

Since $\norm{p_{\ell}}_1 \leq 2\Phi(S;G)/\alpha$, this implies an upper bound on the number of vertices in $S \setminus S_{\beta}$. In particular,
\begin{align}
\label{eqn:zhu_4}
\abs{S \setminus S_{\beta}} \leq \frac{2 \Phi(S;G)/\alpha}{\min_{u \in S} \{T_1(u,\varepsilon)(\deg(u;G[S])/\vol(S;G))\} - \beta\pi_0}. 
\end{align}

\noindent Together \eqref{eqn:zhu_3} and \eqref{eqn:zhu_4} establish an upper bound on the symmetric set difference $\Delta(S_{\beta},S)$ for a general graph $G$, in terms of several graph functionals and input parameters to Algorithm \ref{alg: ppr}. We will move on to establishing bounds on these functionals and parameters in the specific case of $G = G_{n,r}$ and $S = \Csig[\Xbf]$, and the ranges for parameters specified in \eqref{eqn: initialization}. First, however, we verify that the conditions of Lemma \ref{lem: zhu} are satisfied under the setup of Theorem~\ref{thm: misclassification_rate}. Indeed,
\begin{enumerate}
	\item The initialization condition of \eqref{eqn: initialization} explicitly restricts $v \in \Csig[\Xbf]^g$,
	\item By Theorem \ref{thm: mixing_time_upper_bound} along with the initialization condition of \eqref{eqn: initialization}, we have that with probability tending to 1 as $n \to \infty$, $\alpha \leq 1/(9\tau_{\infty}(G_{n,r}[\Csig[\Xbf]]))$,
	\item the \pprspace vector $\pbf(v,\alpha;G_{n,r})$ is simply an approximate \pprspace vector with $\varepsilon = 0$.
\end{enumerate}

All conditions of Lemma~\ref{lem: zhu} are therefore satisfied. We now collect the bounds on graph functionals and algorithm parameters we will use to complete the proof of Theorem~\ref{thm: misclassification_rate}.
\begin{enumerate}
	\item By Theorem 3, with probability tending to $1$ as $n \to \infty$, $\Phi(\Csig[\Xbf]; G_{n,r}) \leq \Phibf(\theta)$. By \eqref{eqn: initialization}, $\alpha \geq 1/(10 \taubf(\theta))$. Therefore, recalling the definition of $\kappa(\Cset)$ in Definition~\eqref{def:well_conditioned_density_cluster}, we have $2 \Phi(\Csig[\Xbf]; G_{n,r})/\alpha \leq 20 \kappa(\Cset)$.
	\item Since $\pbf(v,\alpha;G_{n,r})$ is an exact PPR vector (i.e. an approximate PPR vector with $\varepsilon = 0$), the expression $T_1(u,\varepsilon)$ defined in \eqref{eqn:zhu_5} reduces to $4/5$.
	\item From Lemma~\ref{lem: ball_bounds_in_probability} we have that with probability tending to $1$ as $n \to \infty$, for any $\epsilon > 0$, the following statements hold:
	\begin{equation*}
	\min_{u \in \Csig[\Xbf]} \deg(u; \widetilde{G}_{n,r}) = \degminwt \geq 6/25 n (1 - \epsilon) \lambda_{\sigma} \nu_d r^d
	\end{equation*}
	and 
	\begin{equation*}
	\vol(\Csig[\Xbf];\widetilde{G}_{n,r}) \leq \abs{\Csig[\Xbf]} \degmaxwt \leq \abs{\Csig[\Xbf]} n (1 + \epsilon) \Lambda_{\sigma} \nu_d r^d.
	\end{equation*}
	\item Recalling Algorithm \eqref{alg: ppr}, we have that $\widehat{C} = S_{\beta}$ for some $1/40 \leq \beta \leq 1/12$. Moreover, by the initialization condition of \eqref{eqn: initialization}, $(3/4)(\lambda_{\sigma}/(\Lambda_{\sigma} n \Pbb(\Csig)) \leq \pi_0 \leq (5/4)(\lambda_{\sigma}/(\Lambda_{\sigma} n \Pbb(\Csig))$. Therefore, by \eqref{eqn: Csig_cardinality_bound}, with probability tending to $1$ as $n \to \infty$, $\pi_0$ is contained in the random interval
	\begin{equation*}
	(1/2)(\lambda_{\sigma}/(\Lambda_{\sigma} \abs{\Csig{\Xbf}}) \leq \pi_0 \leq (3/2)(\lambda_{\sigma}/(\Lambda_{\sigma} \abs{\Csig{\Xbf}}).
	\end{equation*}
\end{enumerate}
The latter two points imply that with probability tending to $1$ as $n \to \infty$,
\begin{equation}
\label{eqn:misclassification_rate_proof_1}
4 \degminwt/(5\vol_{n,r}(\Csig[\Xbf])) - \beta\pi_0  \geq \frac{1}{24} \frac{\lambda_{\sigma}}{\Lambda_{\sigma}\abs{\Csig[\Xbf]}}.
\end{equation}
Therefore,
\begin{align}
\Delta(\Cest, \Csig[\Xbf]) & \leq \frac{2 \Phi_{n,r}(\Csig[\Xbf])/\alpha}{\beta \pi_0} + \frac{2 \Phi_{n,r}(\Csig[\Xbf])/\alpha}{4 \degminwt/(5\vol_{n,r}(\Csig[\Xbf])) - \beta\pi_0} \nonumber \\
& \leq \frac{1200 \kappa(\Cset)\Lambda_{\sigma} \abs{\Csig[\Xbf]}}{\lambda_{\sigma}} + \frac{48 \kappa(\Cset) \Lambda_{\sigma} \abs{\Csig{\Xbf}}}{\lambda_{\sigma}}. \label{eqn:misclassification_rate_proof_2}
\end{align}
and the proof of Theorem \ref{thm: misclassification_rate} is complete.

\section{Proof of Theorem \ref{thm: consistent_recovery_of_density_clusters}}
\label{sec: proof_of_consistent_cluster_recovery}

As a reminder, to prove Theorem~\ref{thm: consistent_recovery_of_density_clusters} it will be sufficient to show that under the conditions of that theorem, we have that
\begin{equation}
\label{eqn: PPR_gap}
\min_{u' \in \Cset'[\Xbf]} p_v(u') \leq \beta \pi_0 \leq \min_{u \in \Cset[\Xbf]} p_v(u), \quad \textrm{for every $\beta \in [1/40,1/12]$},
\end{equation}
holds with probability tending to $1$ as $n \to \infty$.

To show this, we first establish in Lemma~\ref{lem: setup} that when Algorithm \ref{alg: ppr} is properly initialized, the resulting PPR vector is large for every vertex $u \in \Cset$, and small for every vertex $u' \in \Cset'$. Let $\overline{\pibf}_{n,r}: \Csig[\Xbf] \to [0,1]$ be the vector given by
\begin{equation*}
...
We immediately note that by assumption, Algorithm \ref{alg: ppr} is well-initialized, and therefore the seed node $v$ is chosen in $\Csig[\Xbf]^g$. Since we additionally assume $\Cset$ is a $\kappa$-well-conditioned density cluster, we have that $r < \sigma/(2d) < \sigma$, and the upper bound $\alpha \leq 1/(9 \tau_{\infty}(G_{n,r}[\Csig[\Xbf]]))$ holds by Theorem \ref{thm: mixing_time_upper_bound}, with probability tending to $1$ as $n \to \infty$.
Therefore, all the conditions of Lemma \ref{lem: setup} are met.

We now collect the estimates on graph functionals and PPR parameters we will use to complete the proof of Theorem~\ref{thm: consistent_recovery_of_density_clusters}. 
\begin{itemize}
	\item By Theorem 3, with probability tending to $1$ as $n \to \infty$, $\Phi(\Csig[\Xbf]; G_{n,r}) \leq \Phibf(\theta)$. By \eqref{eqn: initialization}, $\alpha \geq 1/(10 \taubf(\theta))$. Therefore, recalling the definition of $\kappa(\Cset)$ in Definition~\eqref{def:well_conditioned_density_cluster}, we have $2 \Phi(\Csig[\Xbf]; G_{n,r})/\alpha \leq 20 \kappa(\Cset)$.
	\item By Lemma \ref{lem: ball_bounds_in_probability}, for some $\epsilon > 0$ to be chosen later, 
	\begin{equation*}
	\degminwt, \degminpr \geq (6/25)(1 - \epsilon)nr^d \nu_d \lambda_{\sigma}
	\end{equation*}
	and additionally
	\begin{equation*}
	\widetilde{\vol}_{n,r}(\Csig[\Xbf]) \leq n^2 \Pbb(\Csig) \nu_d r^d \Lambda_{\sigma}
	\end{equation*}
	each with probability tending to $1$ as $n \to \infty$.
	
	\item By the initialization condition of \eqref{eqn: initialization}, $\pi_0 \in [3/4,5/4] \lambda_{\sigma}/(n\Pbb(\Csig)\Lambda_{\sigma})$. 
\end{itemize}

Assuming these bounds hold, for sufficiently small choices of $\epsilon > 0$ and the constant $c$ in the upper bound \eqref{eqn: kappa_ub}, we have that for all $u \in \Cset[\Xbf]$,
\begin{equation}
\label{eqn:proof_consistent_cluster_recovery_2}
p_v(u) \geq \frac{24(1-\epsilon) \lambda_{\sigma}}{125 n \Pbb(\Csig) \Lambda_{\sigma}} - \frac{250 \kappa(\Cset)}{3(1-\epsilon)nr^d\nu_d\lambda_{\sigma}} \geq \frac{5\lambda_{\sigma}}{48\Lambda_{\sigma} \Pbb(\Csig)} \geq \pi_0/12.
\end{equation}
and for all $u' \in \Cset'[\Xbf]$,
\begin{equation}
\label{eqn:proof_consistent_cluster_recovery_1}
p_v(u') \leq \frac{250 \kappa(\Cset)}{3(1-\epsilon)nr^d\nu_d\lambda_{\sigma}} < \frac{3\lambda_{\sigma}}{160\Lambda_{\sigma} \Pbb(\Csig)} \leq \pi_0/40. 
\end{equation}
and \eqref{eqn: PPR_gap} follows. 

We defer the proof of Lemma \ref{lem: setup}, and first extend Theorems~\ref{thm: misclassification_rate} and \ref{thm: consistent_recovery_of_density_clusters} to apply with respect to the aPPR vector.
...
\begin{corollary}
	\label{cor: appr}
	Fix $\lambda > 0$, and let $\Cset \in \Cbb_f(\lambda)$ be a $\kappa$-well-conditioned cluster (with respect to some $\theta$). Choose input parameters $\alpha, r, \pi_0, v$ to be well-initialized in the sense of \eqref{eqn: initialization}, additionally set 
	\begin{equation}
	\label{eqn: appr_parameter}
	\varepsilon = \frac{1}{25 n^2 \Pbb(\Csig) \Lambda_{\sigma} \nu_d r^d}
	\end{equation}
	...
	\begin{enumerate}
		\item The upper bound on symmetric set difference metric $\Delta(\Cest,\Csig[\Xbf])$ given by \eqref{eqn: misclassification_rate_ub} is still valid.
...
\end{corollary}
\begin{proof}
To prove the first claim, note that in the proof of Theorem~\ref{thm: misclassification_rate}, the only time we use the fact that $p(v,\alpha; G_{n,r})$ is an exact PPR vector is to show $T_1(u,\varepsilon) \geq 4/5$. Now, we will show that the choice of $\varepsilon$ given by \eqref{eqn: appr_parameter} results in the upper bound $T_1(u,\varepsilon) \geq 3/5$. As a result, \eqref{eqn:misclassification_rate_proof_1} and therefore \eqref{eqn:misclassification_rate_proof_2} will still hold, up to constants.

To show the desired upper bound on $T_1(u,\varepsilon)$, observe that for any $\epsilon > 0$, with probability tending to $1$ as $n \to \infty$, 
\begin{equation}
\label{eqn: appr_parameter_lb}
\varepsilon \leq \frac{1}{25(1 - \epsilon)\vol_{n,r}(\Csig[\Xbf])},
\end{equation}
and
\begin{equation*}
\max_{u \in \Csig[\Xbf]} \bigl\{ \deg_{n,r}(u) - \widetilde{\deg}_{n,r}(u)\bigr\} \leq (19/25)(1 + \epsilon) \lambda \nu_d r^d.
\end{equation*}
Assuming these bounds hold, we can upper bound $T_1(u,\varepsilon)$ when $G = G_{n,r}$, as follows:
\begin{align*}
T_1(u,\varepsilon) & = \frac{4}{5} - \varepsilon \vol(\Csig[\Xbf];G_{n,r}) \frac{\deg(u;G_{n,r})}{\deg(u; \widetilde{G}_{n,r})} \\
& \geq \frac{4}{5} - \frac{(1 + \epsilon) \lambda \nu_d r^d + \widetilde{\deg}_{n,r}(u)}{25(1 - \epsilon)\widetilde{\deg}_{n,r}(u)} \\
& \geq \frac{4}{5} - \frac{(1 + \epsilon) \lambda \nu_d r^d + \degminwt}{25(1 - \epsilon)\degminwt} \\
& \geq \frac{4}{5} - \frac{(19/25)(1 + \epsilon) + 6/25}{6(1 - \epsilon)}
\end{align*}
and so, for $\epsilon > 0$ sufficiently small, we have $T_1(u,\varepsilon) \geq 3/5$. 
...
To show the desired lower bound on $\min_{u \in \Cset[\Xbf]} p^{(\varepsilon)}_v(u)$, we use the uniform approximation guarantee in \eqref{eqn: appr_error}. Note that for every $u \in \Cset[\Xbf]$, since $r < \sigma$, every point $x \in \Xbf$ connected to $u$ must be contained in $\Csig$. Therefore $\widetilde{\deg}_{n,r}(u) = \deg_{n,r}(u)$. Along with \eqref{eqn: appr_parameter_lb}, this means
\begin{equation*}
\varepsilon \deg_{n,r}(u) \leq \frac{\overline{\pi}_{n,r}(u)}{25(1 - \epsilon)}
\end{equation*}
and therefore by \eqref{eqn: lower_bound_PPR_in_cluster}, for every $u \in \Cset[\Xbf]$,
\begin{equation*}
p^{(\varepsilon)}_v(u) \geq \left\{\frac{4}{5} - \frac{1}{25(1 - \epsilon)}\right\} \overline{\pi}_{n,r}(u) - \frac{20 \Phi_{n,r}(\Csig[\Xbf])/\alpha}{\widetilde{\deg}_{\min}}.
\end{equation*}
\noindent Using the arguments given in the proof of Theorem~\ref{thm: consistent_recovery_of_density_clusters}, and choosing the universal constant in \eqref{eqn: kappa_ub} to be sufficiently small, the upper bound in \eqref{eqn:proof_consistent_cluster_recovery_2} follows.

\end{proof}

\subsection{Proof of Lemma~\ref{lem: setup}}

To state Lemma~\ref{lem: setup}, we introduce some notation. Let $\Dbf$ and $\Wbf$ be the degree and lazy random walk matrices over $G_{n,r}$, and $\widetilde{\Dbf}$ and $\widetilde{\Wbf}$ be the degree and lazy random walk matrices over the induced subgraph $\widetilde{G}_{n,r}$. Given a starting distribution $q$ with $\mathrm{supp}(q) \subseteq \Csig[\Xbf]$, we let
	\begin{equation}
