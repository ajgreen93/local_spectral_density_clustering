...
we provide empirical support our theory.
\end{abstract}
\end{center}
...
which weights points according to their degree in $G_{n,r}$.
\begin{definition}
  ...
  $$
  \Delta(\Cest, \mathcal{S}[\Xbf]) := \vol(\Cest \vartriangle \mathcal{S}[\Xbf];
  G_{n,r}). 
  $$
\end{definition}
...
further work relating this spectra to the geometry of the unlerlying
...
\begin{theorem}
  ...
  is well-initialized with respect to $\Cset$. There exists a set $\Csig[\Xbf]^g 
  \subseteq \Csig[\Xbf]$ of large volume, \smash{$\vol(\Csig[\Xbf]^g; G_{n,r})
    \geq \vol(\Csig[\Xbf];G_{n,r})/2$}, such that the following holds: if 
...
$S_{\beta}$ for $\beta \in (L,U)$ will fulfill both conditions laid out in
...
\begin{theorem}
  ...
  \smash{$\vol(\Csig[\Xbf]^g; G_{n,r}) \geq
    \frac{1}{2}\vol(\Csig[\Xbf];G_{n,r})$}, such that if Algorithm~\ref{alg:
    ppr} is run with any seed node $v \in \Csig[\Xbf]^g$, then for
  $$
  n \geq b_1 (\log n)^{\max\{\frac{3}{d},1\}},
  $$
  the PPR estimated cluster \smash{$\Cest$} satisfies \eqref{eqn:
    consistent_density_cluster_recovery} with probability at least $1-b_2/n$.
\end{theorem}
...
We note that the restriction on $\kappa(\Cset)$ imposed by \eqref{eqn: 
    kappa_ub} results in an upper bound on the symmetric set difference metric
...
\begin{remark}
  ...
  neighborhood graphs such that $r \to 0$, or if this is merely looseness in our  
  upper bounds.
\end{remark}
...
\begin{corollary}
  \label{cor: appr}
  Under the assumptions of Theorem \ref{thm: volume_ssd}, consider instead of
  ...
  \end{equation}
  the upper bound on symmetric set difference in \eqref{eqn: volume_ssd} still
  holds. Further, under the assumptions of Theorem \ref{thm:
    consistent_recovery_of_density_clusters}, the estimator \smash{$\Cest$} is
  still consistent, in the sense of Definition \ref{def:
    consistent_density_cluster_estimation}.  
\end{corollary}
...
\begin{remark}
  Observe that the diameter $\rho$ is absent from Theorem \ref{thm:
    conductance_upper_bound}, in contrast to ultimate bound in Theorem    
...
\begin{theorem}
  ...
  Also assume that \smash{$0 < r \leq \sigma/(2\sqrt{d})$}. Then for
  $$
  n \geq b_1 (\log n)^{\max\{\frac{3}{d},1\}},
  $$
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.495\textwidth]{lower_bound_plots/lower_bound_1}
  \includegraphics[width=0.495\textwidth]{lower_bound_plots/lower_bound_2}
...
\begin{theorem}
  ...
  n \geq \max\set{\frac{64}{\epsilon^2 \rho \sigma \pi r^2},
    \frac{8}{\epsilon}}, 
  $$
  there exists a set $\Cset[X]^g$ of large volume, \smash{$\vol(\Cset[X]^g \cap 
    \Cset^{(1)}[\Xbf]) \geq \frac{1}{10}\vol_{n,r}(\Cset^{(1)}[\Xbf])$}, such
...
Note that $\vol_{n,r}(\Cset^{(1)})$ for large enough $n$ will be of the order
...
our lower bound with respect to PPR run on neighborhood graph, the conclusion is
...
helpful and inspiring conversations. RJT thanks Terrence Tao for that time we
hung out recently, and shared tips on how we each cope with massive fame.
This work was supported in part by the NSF grant DMS-1713003. 
...
\begin{enumerate}
	...
	\item We conclude that the symmetric set distance metric $\Delta(\Cest,\Csig^{(1)}[\Xbf])$ must not be small.
\end{enumerate}
...
\begin{lemma}
	...
	\begin{equation*}
	\Phi_{n,r}(A) \geq \frac{1}{12\pi} \left(1 - 4\frac{\sigma \rho}{r^2 n^2} \vol_{n,r}(A \vartriangle \Cset_{\sigma}^{(1)}[\Xbf]) \right) \frac{\epsilon^2 r}{\sigma}
	\end{equation*}
...
\begin{proof}
	...
	\end{align*}
	and the expected volume of $\Cset_{\sigma}^{(1)}[\Xbf] \cap R_k$ and $\Cset_{\sigma}^{(2)}[\Xbf] \cap R_k$,
	\begin{equation}
	\vol_{\Pbb,r}(\Cset_{\sigma}^{(1)} \cap R_k) = \vol_{\Pbb,r}(\Cset_{\sigma}^{(2)} \cap R_k) \geq \frac{\pi r^3}{2 \sigma \rho^2} \label{eqn:normalized_cut_lb_pf5}
	\end{equation}
	...
	\begin{equation*}	
	\Phi_{n,r}(A) \geq \frac{(1 - \delta)^2}{2(1 + \delta)\pi} \left(1 - 2 \frac{\sigma \rho}{(1 - \delta) r^2 n^2} \vol_{n,r}(A \vartriangle \Cset_{\sigma}^{(1)}[\Xbf]) \right) \frac{\epsilon^2 r}{\sigma},
	\end{equation*}
...
\begin{equation*}
\frac{1}{12\pi} \left(1 - 4\frac{\sigma \rho}{r^2 n^2} \vol_{n,r}(\Cest \vartriangle \Cset_{\sigma}^{(1)}[\Xbf]) \right) \frac{\epsilon^2 r}{\sigma} \leq \Phi_{n,r}(\Cest) \leq \sqrt{89600\left\{\log\left(\frac{\rho \sigma}{\epsilon^2 \pi r^2}\right) + \log 20\right\} \frac{r}{\rho}},
\end{equation*}
Finally, we show that the volume of $\mathcal{L}[\Xbf]^g$ is sufficiently large to ensure that it includes many points in $\Cset_{\sigma}^{(1)}[\Xbf]$:
\begin{align*}
\vol_{n,r}(\mathcal{L}[\Xbf]^g \cap \Cset_{\sigma}^{(1)}[\Xbf]) & \geq  \vol_{n,r}(\mathcal{L}[\Xbf]^g) - \vol_{n,r}((\mathcal{L}[\Xbf]^g \cap (\Cset_{\sigma}^{(0)} \cup \Cset_{\sigma}^{(2)})[\Xbf])[\Xbf]) \\
& \geq \frac{5}{6}\vol_{n,r}(\mathcal{L}[\Xbf]) - \vol_{n,r}((\mathcal{L} \cap (\Cset_{\sigma}^{(0)} \cup \Cset_{\sigma}^{(1)})[\Xbf])[\Xbf]) \\
& \geq \frac{5}{6}\vol_{n,r}((\mathcal{L} \cap \Cset_{\sigma}^{(1)})[\Xbf]) - \frac{1}{6}\vol_{n,r}(\mathcal{L}[\Xbf]) \\
& \geq \left(\frac{5}{6}(1 - \delta) - \frac{1}{2}(1 + \delta)\right)\vol_{\Pbb,r}( \mathcal{L} \cap \Csig^{(1)}) \\
& \geq \frac{(1 - \delta)}{2}\left(\frac{5}{6}(1 - \delta) - \frac{1}{2}(1 + \delta)\right)\vol_{n,r}(\Cset_{\sigma}^{(1)}[\Xbf])
\end{align*}
where the final two inequalities follow from Lemma~\ref{lem:graph_functional_concentration} and hold with probability at least $1 - 3\exp\set{-n\delta^2(\vol_{\Pbb,r}( \mathcal{L} \cap \Csig^{(1)}))^2}$. Setting $\delta = 1/13$, we have that $\vol_{n,r}(\mathcal{L}[\Xbf]^g \cap \Cset_{\sigma}^{(1)}[\Xbf]) \geq \frac{1}{10}\vol_{n,r}(\Cset_{\sigma}^{(1)}[\Xbf])$.
