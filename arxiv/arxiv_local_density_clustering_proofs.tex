...
\begin{lemma}
	...
	Under the conditions of Theorem \ref{thm: consistent_recovery_of_density_clusters}, for any $\epsilon > 0$, the following bounds hold with probability tending to $1$ as $n \to \infty$:
	\begin{align}
	...
	\frac{6}{25}(1 - \epsilon)^2 \Pbb(\Csig) \lambda_{\sigma} \nu_d r^d \leq \frac{\widetilde{\vol}_{n,r}(\Csig[\Xbf])}{n^2} & \leq \Pbb(\Csig) \Lambda_{\sigma} r^d \nu_d (1 + \epsilon)^2 \label{eqn: volume_bounds}
	\end{align}
\end{lemma}
\begin{proof}
	We note that for any $x \in \Csig \cup \Csig'$, by \textcolor{red}{\ref{asmp: bounded_density}} along with Lemma \ref{lem: local_conductance},
	\begin{equation*}
	\frac{6}{25} \lambda_{\sigma} r^d \nu_d \leq \Pbb(B(x,r)) \leq \Lambda_{\sigma} r^d \nu_d.
	\end{equation*}
	...
	\end{equation*}
	As the same bound holds for $\degminpr$, \eqref{eqn: min_degree_bound} is shown, and \eqref{eqn: max_degree_bound} follows from similar reasoning.
	
	\eqref{eqn: Csig_cardinality_bound} follows directly from the law of large numbers, and \eqref{eqn: volume_bounds} from \eqref{eqn: min_degree_bound}, \eqref{eqn: max_degree_bound}, and \eqref{eqn: Csig_cardinality_bound}. 
...
We turn now to the proof of Theorem \ref{thm: consistent_recovery_of_density_clusters}. Let $\piminwt := \min_{u \in \Cset[\Xbf]} \widetilde{\pibf}_{n,r}(u)$, and similarly $\piminpr := \min_{u \in \Cset'[\Xbf]} {\pibf}'_{n,r}(u)$.
\paragraph{Proof of Theorem \ref{thm: consistent_recovery_of_density_clusters}.}
~

Observe that the conditions of Theorem \ref{thm: consistent_recovery_of_density_clusters} subsume the requirements of Lemma \ref{lem: setup}, and therefore by that lemma the following inequalities hold for each $u \in \Cset[\Xbf]$, $u' \in \Cset'[\Xbf]$:
\begin{align}
\pbf(u) & \geq \frac{4}{5} \widetilde{\pibf}_{n,r}(u) - \frac{2 \Phi_{n,r}(\Csig[\Xbf])}{\Psi_{n,r}(\Csig[\Xbf]) \degmaxwt} \nonumber \\
\pbf(u') & \leq \frac{2 \Phi_{n,r}(\Csig[\Xbf])}{\Psi_{n,r}(\Csig[\Xbf]) \degminpr} \label{eqn: setup_redux}
\end{align}
...
\frac{2\widetilde{\Phi}_{n,r}(\Csig[\Xbf])}{\Psi_{n,r}(\Csig[\Xbf]) \widetilde{D}_{\min}},\frac{2\widetilde{\Phi}_{n,r}(\Csig[\Xbf])}{\Psi_{n,r}(\Csig[\Xbf]) \degminpr} \leq \frac{\min \set{\degmaxwt, \degminpr}}{125 \widetilde{\vol}_{n,r}(\Csig[\Xbf])}
\end{equation}
By \eqref{eqn: setup_redux} and \eqref{eqn: gap_bounds}, for each $u \in \Cset[\Xbf]$,
\begin{align*}
\frac{\pbf(u)}{\deg_{n,r}(u)} & \geq \frac{4}{5} \frac{\widetilde{\deg}_{n,r}(u)}{\deg_{n,r}(u) \widetilde{\vol}_{n,r}(\Csig[\Xbf])} - \frac{\degminwt}{125 \deg_{n,r}(u) \widetilde{\vol}_{n,r}(\Csig[\Xbf])} \\
& \geq \frac{22}{125 \widetilde{\vol}_{n,r}(\Csig[\Xbf])} \tag{by Lemma \ref{lem: ball_bounds_in_probability}, applied with $\epsilon = \frac{1}{47}$}
\end{align*}
and by similar reasoning, for each $u' \in \Cset'[\Xbf]$,
\begin{equation*}
\frac{\pbf(u')}{\deg_{n,r}(u')} \leq \frac{\degminpr}{\deg_{n,r}(u') 125 \widetilde{\vol}_{n,r}(\Csig[\Xbf])} \leq \frac{1}{125 \widetilde{\vol}_{n,r}(\Csig[\Xbf])}.
\end{equation*}

\textcolor{red}{Complete the proof.}

It remains to prove \eqref{eqn: gap_bounds}. By application of Lemma \ref{lem: ball_bounds_in_probability} with $\epsilon = 1/11$, $\piminwt \cdot \degminwt$ can be lower bounded
\begin{equation}
\label{eqn: pimin_Dmin_lb}
\piminwt \cdot \degminwt = \frac{\degminwt^2}{\widetilde{\vol}_{n,r}(\Csig[\Xbf])} \geq \frac{1}{25} \frac{\lambda_{\sigma}^2 \nu_d r^d}{\Lambda_{\sigma}\Pbb(\Csig)}  
\end{equation}
where the bound holds with probability tending to one as $n \to \infty$. 

By Theorems \ref{thm: conductance_upper_bound} and \ref{thm: inverse_mixing_time_lower_bound}, with probability tending to one as $n \to \infty$,
\begin{equation}
