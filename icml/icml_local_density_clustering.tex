...
uniformly over $S$.  For this, we first define \emph{relative pointwise mixing
  time} of $G[S]$ as 
$$
\tau_{\infty}(G[S]) = \min\set{ t: \frac{|q_{vu}^{(m)} - 
...
\end{multline}
with probability at least $1 - \delta$, where $c_1,c_2>0$ are constants
...
For PPR to successfully recover density clusters, the ratio
$\Phi_{n,r}(\Csig[\Xbf])/\Psi_{n,r}(\Csig[\Xbf])$ must be small.  Let us
...
For $G = (V,E)$ consider some $A \subseteq V$, and let $\Phi(A; G)$ and $\Psi(A; G)$ be defined as in (\ref{eqn: norm_cut}) and (\ref{eqn: inv_mixing_time}), respectively.r
