...
\begin{enumerate}
	...
	\item \textbf{Sample-to-population results.} For the specific choice of $G = G_{n,r}$, we relate the aforementioned functionals to their population analogues. This proves~\textcolor{red}{(?)} 
	\item \textbf{Bounds on population functionals.} (In the case of density clustering only.) When the candidate cluster is a $\lambda$-density cluster, we bound the population functionals by a function of $\lambda$, as well as the other relevant parameters introduced in Section~\textcolor{red}{(?)}. This proves~\textcolor{red}{(?)}
\end{enumerate}
...
\begin{proof}[Proof (of Lemma~\ref{lem:zhu})]
...
\begin{corollary}
	...
	\end{equation}
\end{corollary}
\begin{proof}[Proof (of Corollary~\ref{cor:zhu})]
...
\end{align*}
Therefore with probability at least $1 - \exp\{-\delta^2 (\cut_{\Pbb,r}(\mc{S}))^2\}$,
\begin{equation*}
\frac{1}{n(n - 1)} \mathrm{cut}_{n,r}(\mc{S}[X]) \leq (1 + \delta) \cut_{\Pbb,r}(\mc{S}),
\end{equation*}
and likewise with probability at least $1 - \exp\{-\delta^2 (\vol_{\Pbb,r}(\mc{S}))^2\}$,
\begin{equation*}
...
\end{equation*}
with probability at least $1 - 3\exp\{-\delta^2 (\cut_{\Pbb,r}(\mc{C}))^2\}$. This establishes~\eqref{eqn:sample_to_population_normalized_cut} upon taking $b_1 := (\cut_{\Pbb,r}(\mc{C}))^2$.

\subsection{Sample-to-population: local spread}
\label{subsec:sample_to_population_local_spread}
In this subsection we establish~\eqref{eqn:sample_to_population_local_spread}. To ease the notational burden, let $\wt{G}_{n,r} := G_{n,r}\bigl[\mc{C}[X]\bigr]$. By Lemma~\ref{lem:hoeffding_2}, with probability at least $1 - \exp\{-2\delta^2 (\Pbb(\mc{C}))^2n\}$
\begin{equation}
\label{pf:sample_to_population_local_spread_1}
\wt{n} \geq (1 - \delta) \cdot n \cdot \Pbb(\mc{C}).
\end{equation}
Conditional on~\eqref{pf:sample_to_population_local_spread_1}, it follows from Lemma~\ref{lem:bernstein_union} that with probability at least $1 -n\exp\{-\delta^2d_{\min}(\wt{\Pbb}) \cdot (1 - \delta) \Pbb(\mc{C}) n/(3 + \delta)\}$,
\begin{equation}
\label{pf:sample_to_population_local_spread_2}
\frac{1}{\wt{n}} d_{\min}(\wt{G}_{n,r}) \geq (1 - \delta) \cdot d_{\min}(\wt{\Pbb}),
\end{equation}
and likewise it follows from Lemma~\ref{lem:hoeffding} that with probability at least $1 - \exp\{-\delta^2(\vol_{\wt{\Pbb},r}(\mc{C}))^2 \cdot (1 - \delta) \Pbb(\mc{C}) n \}$,
\begin{equation*}
\frac{1}{\wt{n}(\wt{n} - 1)} \vol(\wt{G}_{n,r}) \geq (1 - \delta) \cdot \vol_{\wt{\Pbb},r}(\mc{C}).
\end{equation*}
Consequently for any $\delta \in (0,1/3)$,
\begin{equation*}
s_{n,r}(\mc{C}[X]) = \frac{d_{\min}(\wt{G}_{n,r})^2}{\vol(\wt{G}_{n,r})} \geq \frac{\frac{1}{\wt{n}^2}d_{\min}(\wt{G}_{n,r})^2}{\frac{1}{\wt{n}(\wt{n} - 1)}\vol(\wt{G}_{n,r})} \geq \frac{(1 - \delta)^2}{(1 + \delta)} \cdot  \frac{d_{\min}(\wt{\Pbb})^2}{\vol_{\wt{\Pbb},r}(\mc{C})} \geq (1 - 3\delta) \cdot s_{\Pbb,r}(\mc{C}).
\end{equation*}
with probability at least $1 - \exp\{-2\delta^2 (\Pbb(\mc{C}))^2n\} - \exp\bigl\{-2\delta^2\bigl(\vol_{\wt{\Pbb},r}(\mc{C})\bigr)^2 \Pbb(\mc{C})n/3\bigr\} - n\exp\bigl\{-\delta^2 d_{\min}(\wt{\Pbb}) \cdot \Pbb(\mc{C}) n/5\bigr\}$. This establishes~\eqref{eqn:sample_to_population_local_spread} upon taking $b_2 := (\Pbb(\mc{C}))^2 \wedge 2\bigl(\vol_{\wt{\Pbb},r}(\mc{C})\bigr)^2 \Pbb(\mc{C})/3 \wedge d_{\min}(\wt{\Pbb}) \cdot \Pbb(\mc{C})/5$.
...
\begin{theorem}[Theorem~1.1 of \cite{garciatrillos16}]
	...
	Suppose $\Pbb$ satisfies~\ref{asmp:domain} and~\ref{asmp:bounded_density}. Then, there exists positive constants $B_{2}$ and $B_4$ that do not depend on $n$, such that with probability at least $1 - B_2/n$:
	\begin{equation*}
	\Delta_{\infty}(\mbb{P},\mbb{P}_n) \leq B_4 \cdot 
	\begin{dcases*}
...
\end{equation*}
so that we may apply Proposition~\ref{prop:conductance_lb_transportation_distance} to get a lower bound on $\Psi_{n,r}(\mc{C}[X])$ in terms of $\Psi_{\Pbb,r}(\mc{C})$, $\Delta_{\infty}(\wt{\Pbb}, \wt{\Pbb}_n)$, and $d_{\min}(\wt{\Pbb}_{n})$. Next, taking $1 - \delta = 1/\sqrt{2}$ in~\eqref{pf:sample_to_population_local_spread_2}, we have that with probability at least $1 - \exp\{-(\mbb{P}(\mc{C}))^2n/5\} - n\exp\{-d_{\min}(\wt{\Pbb}) \Pbb(\mc{C}) n/50\}$,
\begin{equation*}
...
\begin{equation*}
\Delta_{\infty}(\wt{\Pbb},\wt{\Pbb}_n) \leq B_4 \frac{(\log n)^{p_d}}{n^{1/d}} \leq \min\Bigl\{\frac{b_5}{2\Psi_r(\wt{\Pbb})}, b_5, \frac{r}{4(d - 1)} \Bigr\} \cdot \delta
\end{equation*}
...
\end{equation*}
with probability at least $1 - B_2/n - \exp\{-(\mbb{P}(\mc{C}))^2n/5\} - n\exp\{-d_{\min}(\wt{\Pbb}) \Pbb(\mc{C}) n/50\}$, establishing~\eqref{eqn:sample_to_population_conductance} upon taking $b_3 := (\mbb{P}(\mc{C}))^2/5 \wedge d_{\min}(\wt{\Pbb}) \Pbb(\mc{C}) /50$.
...
Let $\Pbb$ be the hard case distribution over rectangular domain $\mc{X}$, defined as in~\eqref{eqn:lb_density}, and $\mc{L}$ the lower half of $\mc{X}$. Suppose $r \in (0,\sigma/2)$. Then the population normalized cut $\Phi_{\Pbb,r}(\mathcal{L})$ is lower bounded,
\begin{equation}
\label{eqn:ncut_lower_set}
\Phi_{\Pbb,r}(\mc{L}) \leq \frac{8}{3} \cdot \frac{r}{\rho}.
\end{equation}
and the population local spread $s_{\Pbb,r}(\mc{X})$ is upper bounded,
\begin{equation}
\label{eqn:local_spread_ub}
s_{\Pbb,r}(\mc{X}) \geq \frac{\pi r^2 \epsilon^2}{2 \rho\sigma}
\end{equation}

\begin{proof}[Proof of~\eqref{eqn:ncut_lower_set}.]
	...
	\end{align*}
	where the last inequality follows since $r \leq \frac{1}{4}\sigma < \frac{1}{4}\rho$. 
\end{proof}

\begin{proof}[Proof of~\eqref{eqn:local_spread_ub}.]
	\textcolor{red}{(TODO)}
\end{proof}
...
(1 - \delta) \cdot n(n - 1) \vol_{\Pbb,r}(\mc{C}) \leq \vol_{n,r}(\mc{C}[X]) \leq (1 + \delta) \cdot n(n - 1) \vol_{\Pbb,r}(\mc{C});
\end{equation}
(iii) that the number of sample points in $\mc{C}$ is lower bounded,
\begin{equation}
\label{pf:volume_ssd_ub_2}
\wt{n} \geq (1 - \delta) \cdot n \cdot \mbb{P}(\mc{C});
\end{equation}
...
~~\frac{1}{\wt{n}} d_{\min}(\wt{G}_{n,r}) \geq (1 - \delta) \cdot d_{\min}(\wt{\Pbb}),~~\textrm{and}~~\frac{1}{\wt{n}} d_{\max}(\wt{G}_{n,r}) \leq (1 + \delta) \cdot d_{\max}(\wt{\Pbb}).
\end{equation}
By Propositions~\ref{prop:sample_to_population_1}-\ref{prop:sample_to_population_2} and Lemmas~\ref{lem:hoeffding_2}-\ref{lem:bernstein_union}, these inequalities are satisfied with probability at least $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - 2(n + 2)\exp\{-b_2\delta^2n\} - (n + 2)\exp\{-b_3n\}$.

% AJG comment: The presence of a pre-factor of 4 in the second term, instead of 3, accounts for the extra upper bound needed on the vol_{n,r}(\mc{C}[X]). The presence of a pre-factor of 2(n + 2) in the third term, instead of (n + 2), accounts for the extra upper bound needed on d_{\max}(\wt{G}_{n,r}).

\begin{proof}[Proof (of Theorem~\ref{thm:volume_ssd_ub})]
We use Lemma~\ref{lem:zhu} to upper bound $\Delta(\wh{C},\mc{C}[X])$. In order to do so, we must verify that the tuning parameters $\alpha$ and $(L,U)$ satisfy the condition \eqref{eqn:zhu_condition} of this lemma. In order to verify that $\alpha \leq 1/\bigl(2\tau_{\infty}(\wt{G}_{n,r})\bigr)$, we will use Proposition~\ref{prop:conductance_lb_transportation_distance} to upper bound $\tau_{\infty}(\wt{G}_{n,r})$, which we may use because
\begin{equation*}
\frac{d_{\max}(\wt{G}_{n,r})}{\bigl(d_{\min}(\wt{G}_{n,r})\bigr)^2} \leq \frac{(1 + \delta)}{(1 - \delta)^2} \cdot \frac{d_{\max}(\wt{\Pbb})}{\wt{n}\bigl(d_{\min}(\wt{\Pbb})\bigr)^2} \leq \frac{(1 + \delta)}{(1 - \delta)^3} \cdot \frac{d_{\max}(\wt{\Pbb})}{n\Pbb(\mc{C})\bigl(d_{\min}(\wt{\Pbb})\bigr)^2} \leq \frac{1}{16},
\end{equation*}
with the last inequality following by~\eqref{eqn:volume_ssd_ub_sample_complexity} upon taking $B_3 := 16 \cdot d_{\max}(\wt{\Pbb})/\bigl(d_{\min}(\wt{\Pbb})\bigr)^2$ in that equation. Therefore by Proposition~\ref{prop:conductance_lb_transportation_distance}, inequalities~\eqref{eqn:sample_to_population_local_spread} and~\eqref{eqn:sample_to_population_conductance}, and the initialization conditions~\eqref{eqn:initialization} and \eqref{eqn:alpha_initialization}, we have that $\alpha \leq 1/45 \wedge 1/\bigl(2\tau_{\infty}(\wt{G}_{n,r})\bigr)$.  
On the other hand, by the upper bound on $\vol_{n,r}(\mc{C}[X])$ given in~\eqref{pf:volume_ssd_ub_3} and~\eqref{eqn:initialization}, we have that $U \leq 1/\bigl(5 \vol_{n,r}(\mc{C}[X])\bigr)$.
...
From Theorem~\ref{thm:volume_ssd_ub}, we have with probability $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - 2(n + 2)\exp\{-b_2\delta^2n\} - (n + 2)\exp\{-b_3n\}$, there exists a set $\mc{C}_{\lambda,\sigma}[X]^g \subset \mc{C}_{\lambda,\sigma}[X]$ of volume at least $\vol_{n,r}(\mc{C}_{\lambda,\sigma}[X]^g) \geq \vol_{n,r}(\mc{C}_{\lambda,\sigma}[X])/2$, such that
\begin{align*}
...
\begin{lemma}
	...
	With probability at least $1 - \exp\set{-2n\delta^2(\vol_{\Pbb,r}(\mathcal{X}))^2} - \frac{12\sigma \rho}{r^2} \exp\set{-\frac{\delta^2\epsilon r^2 n}{\rho \sigma(3 + \delta)}} - \frac{2\rho}{r}\exp\set{-\frac{\delta^2\pi r^3n}{2\sigma\rho^2(3 + \delta)}}$, the following statement holds:
	\begin{equation*}
...
\end{lemma}
We therefore conclude that $\vol_{n,r}(\wh{C} \vartriangle \mc{C}^{(1)}[X])$ must be large. Now, we give a formal proof Theorem~\ref{thm:ppr_lb}, and then of Lemma~\ref{lem:normalized_cut_lb}.
\begin{proof}[Proof (of Theorem~\ref{thm:ppr_lb})]
	Put
	\begin{align*}
	C_{3,\delta} & := 768 \cdot (1 + 3\delta),&&C_{4,\delta} := \frac{72}{(1 - 3\delta)\pi}, \\
	C_{5,\delta} & := \frac{(1 + \delta)^2}{(1 - \delta)^2},&&C_{6,\delta} := \frac{72(1 + \delta)}{(1 - \delta)} \sqrt{8/3 + 8 \delta}.
	\end{align*}
	...
	\begin{itemize}
		\item Recalling that $n \geq \frac{6 \sigma \rho (1 + \delta)}{(\sigma - 2r)(\rho - r) (1 - \delta)}$ and that $\vol_{\Pbb,r}(\mc{L}) \geq \frac{(\sigma - 2r)(\rho - r)}{2\sigma \rho}$, it follows from Lemmas~\ref{lem:bernstein_union} and~\ref{lem:hoeffding_2} that with probability at least $1 - n\exp\{\delta^2 d_{\min}(\Pbb) /(3 + \delta)\} - \exp\{-\delta^2 \vol_{\Pbb,r}(\mc{L})^2 n\}$,
		\begin{equation}
		\label{pf:ppr_lb_1}
		d_{\max}(G_{n,r}) \leq n \cdot (1 + \delta) d_{\max}(\Pbb) \leq \frac{1}{3}n^2 (1 - \delta) \vol_{\Pbb,r}(\mc{L}) \leq \frac{1}{3}\vol_{n,r}(\mc{L}[X]).
		\end{equation}
		Therefore the lower bound in condition~\eqref{eqn:normalized_cut_ppr_vol} is satisfied.
		\item Noting that $\delta \in (0,1/7)$ implies $\frac{(1 - \delta)}{(1 + \delta)} > \frac{3}{4}$, it follows from Lemma~\ref{lem:hoeffding_2} that with probability at least $1 - 2\exp\{-\delta^2 \vol_{\Pbb,r}(\mc{L})^2 n\}$,
		\begin{equation}
		\label{pf:ppr_lb_2}
		\vol_{n,r}(X) \geq n^2(1 - \delta) \vol_{\Pbb,r}(\mc{X}) \geq 2n^2 (1 - \delta) \vol_{\Pbb,r}(\mc{L}) \geq 2\frac{(1 - \delta)}{(1 + \delta)} \vol_{n,r}(\mc{L}[X]) \geq \frac{3}{2} \vol_{n,r}(\mc{L}[X]).
		\end{equation}
		Therefore the upper bound in condition~\eqref{eqn:normalized_cut_ppr_vol} is satisfied.
		\item It follows from Proposition~\ref{prop:sample_to_population_1} and equations~\eqref{eqn:ncut_lower_set} and~\eqref{eqn:local_spread_ub} that with probability at least $1 - 3\exp\{-\delta^2 (\cut_{\Pbb,r}(\mc{L}))^2n\} - 2\exp\bigl\{-\delta^2\bigl(\vol_{\Pbb,r}(\mc{X}) n\bigr\} - n\exp\bigl\{-\delta^2d_{\min}(\Pbb) n\bigr/(3 + \delta)\}$,
		\begin{equation}
		...
		\end{equation}
		The assumption~\eqref{eqn:ppr_lb_condition} implies that  condition~\eqref{eqn:normalized_cut_ppr_ncut} is satisfied.
	\end{itemize}
	...
	\begin{align*}
	\Phi_{n,r}(\wh{C}) & \overset{(\textrm{Thm.}~\ref{thm:normalized_cut_ppr})}{<} 72\sqrt{\Phi_{n,r}(\mc{L}[X]) \cdot \ln\Bigl(\frac{36}{s_{n,r}(X)}\Bigr)} \\
	& \overset{\eqref{pf:ppr_lb_1},\eqref{pf:ppr_lb_2}}{\leq} 72\sqrt{(1 + 3\delta) \cdot \Phi_{\Pbb,r}(\mc{L}) \cdot \ln\Bigl(\frac{36}{(1 - 3\delta)s_{\Pbb,r}(\mc{X})}\Bigr)} \\
	& \overset{\eqref{eqn:ncut_lower_set}, \eqref{eqn:local_spread_ub}}{\leq} 72\sqrt{(8/3 + 8\delta) \cdot \frac{r}{\rho} \cdot \ln\Bigl(\frac{72 \rho \sigma}{(1 - 3\delta)\pi r^2 \epsilon^2}\Bigr)}.
	\end{align*}	
	...
	\end{equation}
	with probability at least $1 - 5\exp\set{-\delta^2(\cut_{\Pbb,r}(\mathcal{L}))^2n} - n\exp\bigl\{-\delta^2 d_{\min}(\Pbb) n/(3 + \delta)\bigr\} - \frac{12\sigma \rho}{r^2} \exp\set{-\frac{\delta^2\epsilon r^2 n}{\rho \sigma(3 + \delta)}} - \frac{2\rho}{r}\exp\set{-\frac{\delta^2\pi r^3n}{2\sigma\rho^2(3 + \delta)}}$. Solving the inequality~\eqref{pf:ppr_lb_3} for $\vol_{n,r}(\wh{C} \vartriangle \mc{C}^{(1)}[X])$ yields~\eqref{eqn:ppr_lb}. 
	
	We conclude by observing that the set $\mc{L}[X]^g$ must have significant overlap with $\mc{C}^{(1)}[X]$. In particular,
	\begin{align*}
	\vol_{n,r}(\mathcal{L}[X]^g \cap \mc{C}^{(1)}[X]) & \geq  \vol_{n,r}(\mathcal{L}[X]^g) - \vol_{n,r}\bigl((\mathcal{L}[X]^g \cap (\mc{C}^{(0)} \cup \mc{C}^{(2)})[X])\bigr) \\
	& \geq \frac{5}{6}\vol_{n,r}\bigl(\mathcal{L}[X]\bigr) - \vol_{n,r}\bigl((\mathcal{L} \cap (\mc{C}^{(0)} \cup \mc{C}^{(1)}))[X]\bigr) \\
	& \geq \frac{5}{6}\vol_{n,r}\bigl((\mathcal{L} \cap \mc{C}^{(1)})[X]\bigr) - \frac{1}{6}\vol_{n,r}\bigl(\mathcal{L}[X]\bigr) \\
	& \overset{\mathrm{(i)}}{\geq} \left(\frac{5}{6}(1 - \delta) - \frac{1}{2}(1 + \delta)\right)\vol_{\Pbb,r}( \mathcal{L} \cap \mc{C}^{(1)}) \\
	& \overset{\mathrm{(ii)}}{\geq} \frac{(1 - \delta)}{2}\left(\frac{5}{6}(1 - \delta) - \frac{1}{2}(1 + \delta)\right)\vol_{n,r}(\mc{C}^{(1)}[X]) \geq \frac{6}{49} \cdot \vol_{n,r}(\mc{C}^{(1)}[X]),
	\end{align*}
	where $\mathrm{(i)}$ and $\mathrm{(ii)}$ follow from Lemma~\ref{lem:hoeffding} and hold with probability at least $1 - 3\exp\set{-n\delta^2(\vol_{\Pbb,r}( \mathcal{L} \cap \mc{C}^{(1)}))^2}$, and the last inequality follows since $\delta \in (0,1/7)$. The claim of Theorem~\ref{thm:ppr_lb} follows upon taking constants $B_3:= 8 + 12\sigma\rho/r^2 + 2\rho/r$, $b_4 := \cut_{\Pbb,r}(\mc{L})^2 \wedge \epsilon r^2/(\rho \sigma) \wedge \pi r^3/(2\sigma\rho^2) \wedge \vol_{\Pbb,r}( \mathcal{L} \cap \mc{C}^{(1)})^2$ and $b_5 := d_{\min}(\Pbb)$. 
\end{proof}

\begin{proof}[Proof (of Lemma~\ref{lem:normalized_cut_lb}).]
	...
	\vol_{n,r}(Z) \leq \vol_{n,r}(G_{n,r}) \overset{(i)}{\leq} (1 + \delta)  \vol_{\Pbb,r}(\mathcal{X}) n^2 \leq (1 + \delta)\frac{\pi r^2}{\rho \sigma} n^2
	\end{equation}
	where $(i)$ holds with probability at least $1 - \exp\set{-2n\delta^2(\vol_{\Pbb,r}(\mathcal{X}))^2}$, and it turns out this crude upper bound will suffice for our purposes.
	
	We turn to lower bounding $\cut_{n,r}(Z)$. We will approximate the cut of $Z$ by discretizing the space $\mathcal{X}$ into bins, relate the cut of $Z$ to the boundary of the binned set $\overline{Z}$, and then lower bound the size of the boundary of $\overline{Z}$.
	...
	\begin{enumerate}
		...
		\begin{equation*}
		\vol_{n,r}\Bigl( \bigl(A \vartriangle \mc{C}^{(1)}[X]\bigr) \cap R_k \Bigr) \geq \frac{1}{2}\vol_{n,r}(\mc{C}^{(1)}[X] \cap R_k), ~~ \textrm{or}
		\end{equation*}
		\item $\wb{R}_k \cap \wb{Z} = \wb{R}_k$, in which case
		\begin{equation*}
		\vol_{n,r}\Bigl( \bigl(A \vartriangle \mc{C}^{}[X]\bigr) \cap R_k \Bigr) \geq \frac{1}{2}\vol_{n,r}(\mc{C}^{(2)}[X] \cap R_k), ~~\textrm{or}
		\end{equation*}
	...
	\begin{equation*} R_{\min} := \min_{k}\Bigl\{\vol_{n,r}(\mc{C}^{(1)}[X] \cap R_k) \wedge \vol_{n,r}(\mc{C}^{(2)}[X] \cap R_k)\Bigr\}
	\end{equation*} 
	we obtain the following lower bound on the volume of the symmetric set difference,
	\begin{equation}
	\label{eqn:normalized_cut_lb_pf2}
	\vol_{n,r}(A \vartriangle \mc{C}^{(1)}[X]) \geq \frac{1}{2} R_{\min}  \left[\frac{2\rho}{r} - N(R)\right].
	\end{equation}
	...
	\begin{equation}
	\vol_{\Pbb,r}(\mc{C}^{(1)} \cap R_k) = \vol_{\Pbb,r}(\mc{C}^{(2)} \cap R_k) \geq \frac{\pi r^3}{2 \sigma \rho^2} \label{eqn:normalized_cut_lb_pf5}
	\end{equation}
...
In this section, we prove two additional results regarding PPR remarked upon in our main text. In Section~\ref{subsec:appr_volume_ssd_ub}, we show that clustering using the aPPR vector satisfies an equivalent guarantee to Theorem~\ref{thm:volume_ssd_ub}. In Section~\ref{subsec:consistent_recovery_density_clusters}, we show that the PPR vector can perfectly distinguish two distinct density clusters $\mc{C}_{\lambda},\mc{C}_{\lambda}'$. 
...
\begin{corollary}
	...
	holds.
\end{corollary}	
\begin{proof}[Proof (of Corollary~\ref{cor:appr})]
	Note that the choice of $\varepsilon$ in~\eqref{eqn:appr_parameter} implies $\varepsilon \leq 1/25\vol_{n,r}(\mc{C}[X])$ with probability at least $1 - \exp\bigl\{-\delta^2\bigl(\vol_{\Pbb,r}(\mc{C})\bigr)^2\bigr\}$. The proof of Corollary~\ref{cor:appr} is then identical to that of Theorem~\ref{thm:volume_ssd_ub}, except we use Corollary~\ref{cor:zhu} rather than Lemma~\ref{lem:zhu} to relate the symmetric set difference to the graph normalized cut and mixing time.
\end{proof}
...
\begin{equation*}
c_{1,\delta} := \frac{(1 - \delta)^3}{4} \cdot \min\Bigl\{\Bigl(\frac{3}{8(1 + \delta)} - \frac{1}{5}\Bigr),\frac{1}{10}\Bigr\}
\end{equation*}
...
\begin{theorem}
	...
	For any $\delta \in (0,1/4)$, and otherwise under the same conditions as Theorem~\ref{thm:volume_ssd_ub}, the following statement holds with probability at least $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - (n + 2)\exp\{-b_3n\} - 3(n + 3)\exp\{-b_6\delta^2n\}$: there exists a set $\mc{C}_{\lambda,\sigma}[X]^g \subseteq \mc{C}_{\lambda,\sigma}[X]$ of large volume, $\vol_{n,r}(\mc{C}_{\lambda,\sigma}[X]^g) \geq \vol_{n,r}(\mc{C}_{\lambda,\sigma}[X])/2$, such that if Algorithm~\ref{alg:ppr} is $\delta$-well-initialized and run with any seed node $v \in \mc{C}_{\lambda,\sigma}[X]^g$, and moreover
	\begin{equation}
...
\begin{itemize}
	...
	\item The proof of Theorem~\ref{thm:density_cluster_consistent_recovery} relies heavily on Lemma~\ref{lem:ppr_uniform_bound}. This lemma---or more accurately, the equation~\eqref{pf:interpolator_bound_max_entry} used in the proof of the lemma---can be thought of as a smoothness result for the PPR vector, showing that the mass of $p_v(\cdot)$ cannot be overly concentrated at any one node $u$. However,~\eqref{pf:interpolator_bound_max_entry} is a somewhat crude bound. By plugging a stronger result on the smoothness of $p_v(\cdot)$ in to the proof of Lemma~\ref{lem:ppr_uniform_bound}, we could improve the uniform bounds of the lemma in turn show that the conclusion of Theorem~\ref{thm:density_cluster_consistent_recovery} holds under weaker conditions than~\eqref{eqn:density_cluster_consistent_recovery_condition}. 
\end{itemize}  
...
\begin{equation}
n' \geq (1 - \delta) \cdot n \cdot \Pbb(\mc{C}_{\lambda,\sigma}')
\end{equation}
and that 
\begin{equation}
\frac{1}{n'} d_{\min}(G_{n,r}') \geq (1 - \delta) n \cdot d_{\min}(\Pbb').
\end{equation}
By Propositions~\ref{prop:sample_to_population_1}-\ref{prop:sample_to_population_2} and Lemmas~\ref{lem:hoeffding_2}-\ref{lem:bernstein_union}, these inequalities are satisfied with probability at least $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - (n + 2)\exp\{-b_3n\} - 3(n + 3)\exp\{-b_6\delta^2n\}$, taking $b_6 := b_2 \wedge d_{\min}(\Pbb')/4 \wedge \bigl(\Pbb(\mc{C}_{\lambda,\sigma}')\bigr)^2$.

% The pre-factor of 1/4 in the definition of b_6 is to handle the factor of 1/(3 + \delta) in Bernstein's inequality.


\begin{proof}[Proof (of Theorem~\ref{thm:density_cluster_consistent_recovery})]
...
& \geq \frac{1}{n^2}\biggl(\frac{3}{8(1 + \delta)\vol_{\Pbb,r}(\mc{C}_{\lambda,\sigma})} - 4 \cdot \frac{n^2}{\wt{n}^2} \cdot \frac{\kappa_{\Pbb,r}(\mc{C}_{\lambda,\sigma},\delta)}{(1 - \delta) d_{\min}(\wt{\Pbb})^2}\biggr) \\
& \geq \frac{1}{n^2}\biggl(\frac{3}{8(1 + \delta)\vol_{\Pbb,r}(\mc{C}_{\lambda,\sigma})} - 4 \cdot \frac{\kappa_{\Pbb,r}(\mc{C}_{\lambda,\sigma},\delta)}{(1 - \delta)^3 \Pbb(\mc{C})^2 d_{\min}(\wt{\Pbb})^2}\biggr) \\
...
\begin{equation*}
\frac{p_v(u')}{\deg_{n,r}(u')} \leq 2 \frac{\Phi_{n,r}(\mc{C}_{\lambda,\sigma}[X])}{d_{\min}(G_{n,r}')^2 \alpha} \leq 4 \cdot \frac{n^2}{(n')^2} \cdot 4\frac{ \kappa(\mc{C}_{\lambda,\sigma},\delta)}{(1 - \delta) d_{\min}(\Pbb')^2} \leq 4\frac{ \kappa(\mc{C}_{\lambda,\sigma},\delta)}{n^2 (1 - \delta)^3 \Pbb(\mc{C}_{\lambda,\sigma}')^2 d_{\min}(\Pbb')^2} \leq \frac{1}{10 n^2\vol_{\Pbb,r}(\mc{C}_{\lambda,\sigma})},
\end{equation*}
