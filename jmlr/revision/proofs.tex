...
\end{equation}
Conditional on~\eqref{pf:sample_to_population_local_spread_1}, it follows from Lemma~\ref{lem:bernstein_union} that with probability at least $1 -n\exp\{-2(1 - \delta)\delta^2d_{\min}(\wt{\Pbb}) \Pbb(\mc{C}) n\}$,
\begin{equation}
...
\end{equation*}
with probability at least $1 - B_2/n - (n + 2)\exp\{-d_{\min}(\wt{\Pbb})\cdot \Pbb(\mc{C}) n/\sqrt{2}\}$, establishing~\eqref{eqn:sample_to_population_conductance}.
...
\begin{proof}[Proof (of Theorem~\ref{thm:ppr_lb})]
	...
	\begin{itemize}
		...
		\end{equation}
		Therefore by Assumption~\textcolor{red}{(?)}, condition~\eqref{eqn:normalized_cut_ppr_ncut} is satisfied.
	\end{itemize}
	...
	We conclude by observing that the set $\mc{L}[X]^g$ must have significant overlap with $\mc{C}^{(1)}[X]$: 
	\begin{align*}
	\vol_{n,r}(\mathcal{L}[X]^g \cap \mc{C}^{(1)}[X]) & \geq  \vol_{n,r}(\mathcal{L}[X]^g) - \vol_{n,r}((\mathcal{L}[X]^g \cap (\mc{C}^{(0)} \cup \mc{C}^{(2)})[X])[X]) \\
...
\mathbb{C}_f(\lambda)$. \textcolor{red}{TODO: explain why this is a desirable property for a cluster estimate $\wh{C}$.}
...
\end{theorem}
In one sense, this is a strong result: if the density cluster $\mc{C}_{\lambda}$ satisfies the requirement~\eqref{eqn:density_cluster_consistent_recovery_condition}, and we are willing to ignore the behavior of the algorithm in low-density regions, Corollary~\ref{cor:density_cluster_volume_ssd_ub} guarantees that PPR will \emph{perfectly distinguish} the candidate cluster $\mc{C}_{\lambda}$ from all other connected components $\mc{C}_{\lambda}' \in \mathbb{C}_f(\lambda), \mc{C}_\lambda' \not= \mc{C}_{\lambda}$.
