...
\end{align*}
Finally, we claim that under Assumption~\ref{asmp:low_noise_density}, for any $r \leq \sigma/4d$ the $(\sigma + r)$-thickened set $\mc{C}_{\lambda,\sigma + r}$ satisfies the following isoperimetric inequality:
\begin{equation}
\label{pf:density_cluster_normalized_cut}
\Pbb\bigl(\mc{C}_{\lambda,\sigma + r} \setminus \mc{C}_{\lambda,\sigma}\bigr) \leq \frac{dr}{\sigma - dr} \cdot \left(\lambda_{\sigma} - \theta\frac{r^{\gamma}}{\gamma + 1}\right) \cdot \nu(\mc{C}_{\lambda,\sigma}),
\end{equation}
whence the claim of Proposition~\ref{prop:density_cluster_normalized_cut} follows.

\paragraph{Proof of~\eqref{pf:density_cluster_normalized_cut}.}
For conciseness, we denote $\mc{B}_{\lambda,r} := \mc{C}_{\lambda,\sigma + r} \setminus \mc{C}_{\lambda,\sigma}$. To upper bound $\Pbb(\mc{B}_{\lambda,r})$, we begin by partitioning $\mc{B}_{\lambda,r}$ into slices based on distance from $\mc{C}_{\lambda,\sigma}$ as follows: for $k \in \mathbb{N}$,
\begin{equation*}
\mathcal{T}_{i,k} = \set{x \in \mc{B}_{\lambda,r}: t_{i,k} < \frac{\mathrm{dist}(x, \mc{C}_{\lambda,\sigma})}{r} \leq t_{i+1,k}}, ~~ \mc{B}_{\lambda,r} = \bigcup_{i = 0}^{k-1} \mathcal{T}_{i,k},
\end{equation*}
where $t_{i,k} = i/k$ for $i = 0, \ldots, k - 1$. As a result, for any $k \in \mathbb{N}$,
\begin{equation}
\label{eqn: partition_ub}
\Pbb(\mc{B}_{\lambda,r}) = \int_{\mc{B}_{\lambda,r}} f(x) \dx = \sum_{i = 0}^{k-1} \int_{\mathcal{T}_{i,k}} f(x) \dx \leq \sum_{i = 0}^{k-1} \Bigl(\nu(\mathcal{T}_{i,k}) \cdot \max_{x \in \mathcal{T}_{i,k}}\{f(x)\}\Bigr)
\end{equation}
Assumptions~\ref{asmp:lambda_bounded_density} and \ref{asmp:low_noise_density} imply the upper bound
\begin{equation*}
\max_{x \in \mathcal{T}_{i,k}} f(x) \leq \lambda_{\sigma} - \theta(rt_{i,k})^{\gamma},
\end{equation*}
and writing
\begin{equation*}
\nu(\mathcal{T}_{i,k}) = \nu(\mc{C}_{\lambda,\sigma} + rt_{i+1,k}B) - \nu(\mc{C}_{\lambda,\sigma} + rt_{i,k}B) =: \nu_{i+1,k} - \nu_{i,k},
\end{equation*}
we have
\begin{align}
...
\end{align}
and we now move to upper bound $\Sigma_k$ and $\xi_k$.

\textcolor{red}{(TODO)}.

We first upper bound $\xi_k$, since it is the simpler of the two to upper bound, and the technique we use will be useful in the upper bound on $\Sigma_k$ as well.

We first consider the term $\Sigma_k$. $\mc{C}$ has finite diameter by Assumption~\ref{asmp: bounded_density}, as otherwise $\int_{\mc{C}_{\lambda,\sigma}} f(x) dx = \infty$. Letting $\wb{\mc{C}}$ be the closure of $\mc{C}$, we observe that $\wb{\mc{C}_{\lambda,\sigma}} = \wb{\mc{C}} + \sigma B$, and moreover for any $\delta > 0$, $\nu(\wb{\mc{C}_{\lambda,\sigma}} + \delta B) = \nu(\mc{C}_{\lambda,\sigma} + \delta B)$ (as the boundary $\partial(\mc{C}_{\lambda,\sigma} + \delta B)$ is Lipschitz and therefore has measure zero). As a result, for each $t_{i,k}, i = 1, \ldots,k$ we may apply Lemma \ref{lem: expansion_volume} to $\wb{\mc{C}}$ and obtain
\begin{equation}
\label{eqn: slice_volume_bound}
\nu_{i,k} = \nu(\mc{C}_{\lambda,\sigma} + rt_{i,k}B) \leq \nu(\mc{C}_{\lambda,\sigma})\left(1 + \frac{rt_{i,k}}{\sigma}\right)^d
\end{equation}
which in turn gives
\begin{align}
\Sigma_k & \leq \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \sum_{i = 1}^{k} \left(1 + \frac{ rt_{i,k}}{\sigma}\right)^d \biggl( (t_{i,k})^{\gamma} - (t_{i-1,k})^{\gamma}\biggr) \nonumber \\
& = \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \sum_{i = 1}^{k} \left(1 + \frac{ru_{i,k}^{1/\gamma}}{\sigma}\right)^d ( u_{i,k} - u_{i,k-1}).~~~~~~~~~~~~~~ (\text{substituting}~u_{i,k} := t_{i,k}^{\gamma}) \label{eqn: Sigmak_riemann_sum}
\end{align}
The expression in~\eqref{eqn: Sigmak_riemann_sum} is a Riemann sum, and taking the limit as $k \to \infty$ we obtain
\begin{align}
\lim_{k \to \infty} \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \sum_{i = 1}^{k} \left(1 + \frac{ru_{i,k}^{1/\gamma}}{\sigma}\right)^d ( u_{i,k} - u_{i,k-1}) & = \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \int_{0}^{1} \left(1 + \frac{r u^{1/\gamma}}{\sigma}\right)^{d} du \nonumber \\
& \overset{\text{(i)}}{\leq} \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \int_{0}^{1} \left(1 + \frac{ d r u^{1/\gamma}}{\sigma - dr}\right) du \nonumber \\
& = \theta\nu(\mc{C}_{\lambda,\sigma}) r^\gamma \left(1 + \gamma \frac{d r}{(\gamma + 1)(\sigma - dr)}\right). \label{eqn: Sigmak_integral}
\end{align}
...
\begin{align}
\xi & \overset{\text{(ii)}}{\leq} \nu(\mc{C}_{\lambda,\sigma}) \biggl\{ \left(1 + \frac{ r}{\sigma}\right)^d(\lambda_{\sigma} - \theta r^{\gamma}) - \lambda_{\sigma} \biggr\} \nonumber \\
& \overset{\text{(iii)}}{\leq} \nu(\mc{C}_{\lambda,\sigma}) \biggl\{ \left(1 + \frac{d r}{\sigma - dr}\right)(\lambda_{\sigma} - \theta r^{\gamma}) - \lambda_{\sigma} \biggr\} = \nu(\mc{C}_{\lambda,\sigma}) \biggl\{ \frac{dr}{\sigma - dr}(\lambda_{\sigma} - \theta r^{\gamma}) - \theta r^{\gamma} \biggr\}. \label{eqn: xi_ub}
\end{align}
