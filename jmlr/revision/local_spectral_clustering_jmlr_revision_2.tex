...
\subsection{PPR clustering, and worst-case guarantees}
...
\begin{itemize}
	...
	\item In Section~\ref{subsec:lower_bound}, we prove a negative result: we give a hard distribution $\Pbb$ with corresponding density cluster $\mc{C}_{\lambda}$ for which the symmetric set difference $\Delta(\wh{C},\mc{C}_{\lambda}[X])$ is provably large.
\end{itemize}
...
\begin{proposition}
	...
	Fix $\delta \in (0,1/3)$. Suppose $\mc{C}$ and $\wt{\Pbb}$ satisfy~\ref{asmp:domain} and~\ref{asmp:bounded_density}. There exist positive constants $b_1$-$b_3$ such that each of the following statements hold.
	\begin{itemize}
...
\begin{proposition}
	...
	Fix $\delta \in (0,1/2)$. Suppose $\mc{C}$ and $\wt{\Pbb}$ satisfy~\ref{asmp:domain} and~\ref{asmp:bounded_density}. Then there exist positive constants $B_1$ and $B_2$, such that for any $n \in \mathbb{N}$ for which
	\begin{equation}
...
\alpha_{\Pbb,r}(\mc{C},\delta) := \min\biggl\{\frac{\ln(2)}{13} \cdot \frac{\Psi_{\Pbb,r}^2(\mc{C})}{\ln^2\Bigl(\frac{(1 - 3\delta)s_{\Pbb,r}(\mc{C})}{8}\Bigr)}, \frac{1}{45}\biggr\}
\end{equation}
...
\begin{itemize}
	...
	\ref{thm:volume_ssd_ub} leads to an analogous result which holds for the aPPR vector \smash{$p_v^{(\varepsilon)}$}.
	\begin{corollary}
		\label{cor: appr}
		Consider instead of
...
We start with some background on density clustering. For a given threshod $\lambda \in (0,\infty)$, let \smash{$\mathbb{C}_f(\lambda)$} denote the connected components of the density upper level set $\{x \in \Rd: f(x) > \lambda\}$. In the density clustering problem, initiated by~\cite{hartigan1975}, the goal is to recover $\mathbb{C}_{f}(\lambda)$. By now, density clustering (and the related problem of level-set estimation) are relatively well-understood. \citet{polonik1995,rigollet2009} study density clustering under the symmetric set difference
...
\noindent Applying these results along with Theorem~\ref{thm:volume_ssd_ub}, we obtain an upper bound on $\Delta(\wh{C},\mc{C}_{\lambda,\sigma}[X])$. In the following, $C_{1,\delta}$ and $C_{2,\delta}$ are constants which may depend only on $\delta$, but not $n$, $\Pbb$ or $\mc{C}_{\lambda,\sigma}$.
