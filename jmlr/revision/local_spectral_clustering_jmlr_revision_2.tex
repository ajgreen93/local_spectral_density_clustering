...
Before giving a more detailed overview of our main results, we review some of the aforementioned worst-case guarantees, formally define PPR on a neighborhood graph, and introduce the population-level functionals that govern the behavior of local clustering in our statistical context. 
...
Once $p_v$ or $p_v^{(\varepsilon)}$ is computed, the cluster estimate $\wh{C}$ is chosen by taking a particular sweep cut of $p_v$. For a given level $\beta > 0$, the \emph{$\beta$-sweep cut} of $p_v = (p_v(u))_{u \in V}$ is 
\begin{equation}
...
\textcolor{red}{(TODO): Possibly add a section here \emph{giving}---rather than summarizing---our main results.}
...
We conclude this section by summarizing some related work (in addition to the background already given above). 
...
consistent estimation of the cluster tree $\{\mathbb{C}_f(\lambda): \lambda \in (0,\infty)\}$. \textcolor{red}{Add, at a minimum, references to Larry + Ale and Ingo's papers.}

We emphasize that our goal is
not to improve on these results, nor to offer a better algorithm for level set
estimation, but rather to better understand the implications of our general theory by applying it within an already well-studied framework. Indeed, seen as a density clustering algorithm, PPR has none 
of the optimality guarantees found in the aforementioned works. We should also note that since we study a local algorithm, our interest will be in a local version of the density clustering problem, where the goal is to recover a single density cluster $\mc{C}_{\lambda} \in \mathbb{C}_f(\lambda)$. 
...
\end{equation}
where $\deg_{\wt{\Pbb},r}(x) := \int_{\Rd} \1\{\|y - x\| \leq r\} \,d\wt{\Pbb}(y)$ is the expected degree of $x$ in $G_{n,r}\bigl[\mc{C}[X]\bigr]$. 
...
\begin{enumerate}[label=(A\arabic*)]
	...
	\end{equation*}
\end{enumerate}
In what follows, we use $b_1,b_2,\ldots$ and $B_1,B_2,\ldots$ to refer to constants that may depend on $\Pbb$ and $\mc{C}$, but do not depend on $n$ or $\delta$. We explicitly keep track of all constants in our proofs.
\begin{proposition}
	\label{prop:sample_to_population_1}
	Fix $\delta \in (0,1/3)$. Suppose $\mc{C}$ and $\wt{\Pbb}$ satisfy~\ref{asmp:domain} and~\ref{asmp:bounded_density}. There exist positive constants $b_1$ and $b_2$ such that each of the following statements hold.
	\begin{itemize}
		...
		\end{equation}
		\item With probability at least $1 - (n + 2)\exp\{-b_2\delta^2n\}$,
		\begin{equation}
...
\begin{proposition}
	...
	Fix $\delta \in (0,1/2)$. Suppose $\mc{C}$ and $\wt{\Pbb}$ satisfy~\ref{asmp:domain} and~\ref{asmp:bounded_density}. Then there exist positive constants $B_1$,$B_2$, and $b_3$ such that for any $n \in \mathbb{N}$ for which
	\begin{equation}
	\label{eqn:sample_to_population_conductance_sample_complexity}
	\frac{(\log n)^{p_d}}{n^{1/d}} \leq \delta \cdot B_1,
	\end{equation}
	the following inequality holds with probability at least $1 - B_2/n - (n + 2)\exp\{-b_3n\}$:
	\begin{equation}
...
\alpha_{\Pbb,r}(\mc{C},\delta) := \frac{\ln(2)}{13} \cdot \frac{\Psi_{\Pbb,r}^2(\mc{C})}{\ln^2\Bigl(\frac{8}{(1 - 3\delta)s_{\Pbb,r}(\mc{C})}\Bigr)} 
\end{equation}
...
\begin{theorem}
	...
	Let $\mc{C} \subseteq \Rd$ and $\delta \in (0,1/4)$, and suppose that
	\begin{equation}
	\label{eqn:volume_ssd_ub_sample_complexity}
	\frac{(\log n)^{p_d}}{n^{1/d}} \leq \delta \cdot B_1,~~\textrm{and}~~\frac{(1 + \delta)}{(1 - \delta)^3} \cdot B_3 \leq n.
	\end{equation} 
	With probability at least $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - 2(n + 2)\exp\{-b_2\delta^2n\} - (n + 2)\exp\{-b_3n\}$, the following statement holds: there exists a set $\mc{C}[X]^g \subseteq \mc{C}[X]$ of large volume, $\vol_{n,r}(\mc{C}[X]^g) \geq \vol_{n,r}(\mc{C}[X])/2$, such that if Algorithm~\ref{alg:ppr} is $\delta$-well-initialized with respect to $\mc{C}$, and run with any seed node $v \in \mc{C}[X]^g$, then the PPR estimated cluster $\wh{C}$ satisfies
	\begin{equation}
...
\end{equation*}
We now make some other remarks:
\begin{itemize}
	...
	\textcolor{red}{(TODO): Add a comment explaining why we get different bounds.}
...
\begin{theorem}
	...
	Let $C_{\lambda} \subseteq \Rd$ and $\delta \in (0,1/4)$. Suppose that $C_{\lambda,\sigma}$ satisfies Assumptions~\ref{asmp:lambda_bounded_density}-\ref{asmp:bounded_volume} for some $\lambda_{\sigma}, \Lambda_{\sigma}, \theta, \gamma, \rho$ and $L$, that $0 < r \leq \sigma/4d$, and that $n$ satisfies~\eqref{eqn:volume_ssd_ub_sample_complexity}. Then with probability at least $1 - B_2/n - 4\exp\{-b_1\delta^2n\} - 2(n + 2)\exp\{-b_2\delta^2n\} - (n + 2)\exp\{-b_3n\}$, the following statement holds: there exists a set $\mc{C}[X]^g \subseteq \mc{C}[X]$ of large volume, $\vol_{n,r}(\mc{C}[X]) \geq \vol_{n,r}(\mc{C}[X])/2$, such that if Algorithm~\ref{alg:ppr} is $\delta$-well-initialized with respect to $\mc{C}$, and run with any seed node $v \in \mc{C}[X]^g$, then the PPR estimated cluster $\wh{C}$ satisfies
	\begin{equation}
...
\end{equation}
In the following Theorem, the samples $x_1,\ldots,x_n$ are drawn from the distribution $\Pbb$ with density $f$ given by~\eqref{eqn:lb_density}. \textcolor{red}{(TODO): Clean this up.}
\begin{theorem}
	\label{thm:ppr_lb}
	Fix $\delta \in (0,1/7)$. Assume the neighborhood graph radius $r < \sigma/2$, that
	\begin{equation}
	\label{eqn:ppr_lb_condition}
	\max\biggl\{C_{3,\delta} \cdot \frac{r}{\rho} \cdot \ln\Bigl(C_{4,\delta} \frac{\rho\sigma}{r^2 \epsilon^2} \Bigr), \frac{C_{5,\delta}}{n} \cdot \frac{\rho \sigma}{r^2 \epsilon^2} \biggr\} < \frac{1}{18}~~\textrm{and}~~n \geq \frac{6 \sigma \rho (1 + \delta)}{(\sigma - 2r)(\rho - r) (1 - \delta)},
	\end{equation} 
	and that Algorithm~\ref{alg:ppr} is initialized using inputs $\alpha = 36 \cdot   
	\Phi_{n,r}(\mathcal{L}[X])$, and $(L,U) = (0,1)$.  Then the following statement holds with probability at least $1 - B_3 \exp\set{- \delta^2 b_4 n/(3 + \delta)} - n \exp\{-\delta^2 b_5 n/(3 + \delta)\}$: there exists a set $\mc{C}[X]^g$ of large volume, \\ \smash{$\vol_{n,r}(\mc{C}[X]^g \cap 
		\mc{C}^{(1)}[X]) \geq 6\vol_{n,r}(\mc{C}^{(1)}[X];G_{n,r})/49$}, such
	...
	\mc{C}^{(1)}[X]) \geq \frac{1 - \delta}{2} -  C_{6,\delta} \cdot 
	\frac{\sqrt{\sigma/\rho}}{\epsilon^2} \cdot \sqrt{ \log\left(C_{4,\delta} \cdot \frac{\rho \sigma}
		{\epsilon^2 r^2}\right) \frac{\sigma}{r}},   
	\end{equation} 
\end{theorem}

\textcolor{red}{(TODO): Check that the assumptions of the Theorem match those used in the proof.}
...
\begin{itemize}
	...
	to $\Reals^d$ and to non-rectangular clusters.
	...
	\item Technically, the rectangles $\mc{C}^{(0)},\mc{C}^{(1)},\mc{C}^{(2)}$ are not
...
We provide numerical experiments to investigate the tightness of our theoretical results in Section~\ref{sec:ppr_density_cluster}, and examine the performance of PPR on the ``two moons'' dataset. We defer details of the experimental settings to the appendix.   

\paragraph{Validating theoretical bounds.}
We investigate the tightness of Lemma~\ref{lem:density_cluster_local_spread} and Propositions~\ref{prop:density_cluster_normalized_cut} and \ref{prop:density_cluster_conductance} via simulation. Figure \ref{fig:bounds} compares our bounds on normalized cut, conductance, and local spread of a density cluster with the actual empirically-computed quantities. In the first row we vary the diameter $\rho$ of the candidate cluster, in the second row we vary the thickness $\sigma$ of the candidate cluster, and in the last row we vary the magnitude $\theta$ of the density drop. In each case, we see that our bounds track closely with their empirical counterparts, and are loose by roughly an order of magnitude or less. The looseness in each of these bounds will propagate to our eventual upper bound on $\Delta(\wh{C},\mc{C}_{\sigma}[X])/\vol_{n,r}(\mc{C}_{\sigma}[X])$, which as a result is loose by several orders of magnitude. Nevertheless, for sufficiently well-conditioned enough densities our upper bound will be $\ll 1$; for instance, when $\sigma = \textcolor{red}{(?)}$, $\rho = \textcolor{red}{(?)}$ and $\theta = \textcolor{red}{(?)}$.\\

\textcolor{red}{Alden for Ryan: the last couple of sentences are an attempt at what we discussed in our meeting. I am not satisfied with it, but could not do better.}
...
In another direction, it would be very useful to find reasonable conditions under which the ratio $\Delta(\wh{C},\mc{C}[X])/\vol_{n,r}(\mc{C}[X])$ would tend to $0$ as $n \to \infty$. It seems likely that such a strong result would entail bounds on the $\Leb^{\infty}$-error of PPR. Although most results thus far derive bounds on the $\Leb^1$- or $\Leb^2$-error of spectral clustering methods, some recent works~\citep{dunson2020,calder2020} have established $\Leb^{\infty}$-bounds on the error with which the eigenvectors of a graph Laplacian matrix approximate the eigenvectors of a weighted Laplace-Beltrami operator. It is not clear whether the techniques used in these works can be applied to PPR.

Finally, after reading our Section~\ref{sec:ppr_density_cluster}, the practitioner may be tempted to ask why they should estimate a density cluster using PPR, when other methods are available with superior statistical guarantees. A undoubtedly correct, if perhaps unsatisfying, answer is that the ``ideal'' cluster is entirely in the eye of the beholder. For instance, it is quite common to define an ideal cluster as one with a small normalized cut, in which case the cluster output by PPR may be quite preferable to standard density clustering procedures. On the other hand, it would also be interesting to know whether a (local) spectral method such as PPR converges more rapidly (as a function of $n$) than standard density clustering procedures. Such a question could be answered by comparing the sample complexity of spectral clustering methods with that of density clustering procedures, but to our knowledge, the sample complexity of spectral methods remains unknown.
