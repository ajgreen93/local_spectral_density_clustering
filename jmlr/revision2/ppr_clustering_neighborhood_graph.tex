...
there exists a set $\mc{C}^{(1)}[X]^g \subseteq \mc{C}^{(1)}[X]$ with
\smash{$\vol_{n,r}(\mc{C}^{(1)}[X]^g) \geq \frac{1}{2}\vol_{n,r}(\mc{C}^{(1)}[X])$} such
that for any seed node $v \in \mc{C}^{(1)}[X]^g$, and for large enough $n$, the
